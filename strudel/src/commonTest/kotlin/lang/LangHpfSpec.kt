...
class LangHpfSpec : StringSpec({
    ...
    "hpf() with continuous pattern sets cutoffHz correctly" {
        ...
        // t=0.0: sine(0) = 0.5
        events[0].data.filters.getByType<FilterDef.HighPass>()?.cutoffHz shouldBe (0.5 plusOrMinus EPSILON)
        // t=0.25: sine(0.25) = 1.0
        events[1].data.filters.getByType<FilterDef.HighPass>()?.cutoffHz shouldBe (1.0 plusOrMinus EPSILON)
        // t=0.5: sine(0.5) = 0.5
        events[2].data.filters.getByType<FilterDef.HighPass>()?.cutoffHz shouldBe (0.5 plusOrMinus EPSILON)
        // t=0.75: sine(0.75) = 0.0
        events[3].data.filters.getByType<FilterDef.HighPass>()?.cutoffHz shouldBe (0.0 plusOrMinus EPSILON)
    }
})
