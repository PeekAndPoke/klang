...
class LangAddSpec : StringSpec({
    ...
    "add() works as top-level function with (amount, pattern)" {
        val p = add("2", n("0 1"))
        val events = p.queryArc(0.0, 1.0)
    ...
    "add() works with scale logic when placed before scale" {
        // n("0 2").add("1").scale("C4:major")
        // n("0 2") -> 0, 2
        // add("1") -> 1, 3
        // scale uses these values as degrees
        // 1 -> D4, 3 -> F4
        val p = n("0 2").add("1").scale("C4:major")
        val events = p.queryArc(0.0, 1.0)
    ...
    "add() works in compiled code" {
        val p = StrudelPattern.compile("""n("0 1").add("2")""")
