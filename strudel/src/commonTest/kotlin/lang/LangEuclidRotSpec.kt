...
class LangEuclidRotSpec : StringSpec({
    ...
    "euclidRot dsl interface" {
        ...
        dslInterfaceTests(
            ...
            "script pattern.euclidRot(3, 8, 1)" to StrudelPattern.compile("""s("$pat").euclidRot(3, 8, 1)"""),
            "string.euclidRot(3, 8, 1)" to pat.euclidRot(3, 8, 1),
            "script string.euclidRot(3, 8, 1)" to StrudelPattern.compile(""""$pat".euclidRot(3, 8, 1)"""),
            "euclidRot(3, 8, 1)" to s(pat).apply(euclidRot(3, 8, 1)),
            "script euclidRot(3, 8, 1)" to StrudelPattern.compile("""s("$pat").apply(euclidRot(3, 8, 1))"""),
        ) { _, events ->
