...
/**
 * Rational2 number representation using numerator/denominator arithmetic.
 *
 ...
 *
 * Inspired by Fraction.js, this provides precise Rational2 arithmetic without floating-point
 * precision loss.
 */
@ConsistentCopyVisibility
@Serializable(with = Rational2Serializer::class)
data class Rational2 private constructor(
    val numerator: Long,
    val denominator: Long,
) : Comparable<Rational2> {

    companion object {
        /** Zero as a Rational2 number */
        val ZERO = Rational2(0L, 1L)

        /** One as a Rational2 number */
        val ONE = Rational2(1L, 1L)

        /** Minus One as a Rational2 number */
        val MINUS_ONE = Rational2(-1L, 1L)

        /** NaN as a Rational2 number */
        val NaN = Rational2(0L, 0L)
        ...
        /**
         * Creates a simplified Rational2 number from numerator and denominator.
         * Automatically reduces the fraction and normalizes the sign.
         */
        private fun create(num: Long, den: Long): Rational2 {
            if (den == 0L) return NaN
            if (num == 0L) return ZERO

            // Optimization for identity
            if (den == 1L) return Rational2(num, 1L)
            ...
            if (sDen < 0) {
                ...
                // This implies the Rational2 number cannot be represented with a positive denominator in Long.
            ...
            return Rational2(sNum, sDen)
        }

        /** Creates a Rational2 from a [Long] */
        operator fun invoke(value: Long): Rational2 = Rational2(value, 1L)

        /** Creates a Rational2 from an [Int] */
        operator fun invoke(value: Int): Rational2 = invoke(value.toLong())

        /**
         * Creates a Rational2 from a [Double] using the Continued Fraction algorithm.
         * This finds the best Rational2 approximation that fits within [Long.MAX_VALUE].
         */
        operator fun invoke(value: Double): Rational2 {
        ...
        /** Extension to convert any [Number] to a Rational2 */
        fun Number.toRational(): Rational2 = when (this) {
            is Long -> Rational2(this)
            is Int -> Rational2(this.toLong())
            is Float -> Rational2(this.toDouble())
            is Double -> Rational2(this)
            else -> Rational2(this.toDouble())
        }
    }
    ...
    /** Adds two Rational2 numbers: a/b + c/d */
    operator fun plus(other: Rational2): Rational2 {
    ...
    /** Subtracts another Rational2 number from this one */
    operator fun minus(other: Rational2): Rational2 = plus(-other)

    /** Multiplies two Rational2 numbers: (a/b) * (c/d) */
    operator fun times(other: Rational2): Rational2 {
        if (isNaN || other.isNaN) return NaN
    ...
    /** Divides this Rational2 by another: (a/b) / (c/d) = (a/b) * (d/c) */
    operator fun div(other: Rational2): Rational2 {
    ...
    /** Computes the remainder of division between two Rational2 numbers */
    operator fun rem(other: Rational2): Rational2 {
        if (isNaN || other.isNaN || other.numerator == 0L) return NaN
    ...
    /** Negates the Rational2 number */
    operator fun unaryMinus(): Rational2 = if (isNaN) NaN else create(-numerator, denominator)

    // --- Number Interoperability ---

    operator fun plus(other: Number): Rational2 = this + other.toRational()
    operator fun minus(other: Number): Rational2 = this - other.toRational()
    operator fun times(other: Number): Rational2 = this * other.toRational()
    operator fun div(other: Number): Rational2 = this / other.toRational()
    operator fun rem(other: Number): Rational2 = this % other.toRational()

    // --- Comparison ---

    override operator fun compareTo(other: Rational2): Int {
    ...
    /** Converts the Rational2 number to a [Double] */
    ...
    /** Returns the absolute value of this Rational2 */
    fun abs(): Rational2 {
        if (isNaN) return NaN
        if (numerator == Long.MIN_VALUE) return NaN
        return if (numerator < 0) Rational2(-numerator, denominator) else this
    }

    /** Returns the largest integer (as Rational2) less than or equal to this value */
    fun floor(): Rational2 {
        if (isNaN) return NaN
        ...
        return Rational2(result, 1L)
    }

    /** Returns the smallest integer (as Rational2) greater than or equal to this value */
    fun ceil(): Rational2 {
        if (isNaN) return NaN
        val f = floor()
