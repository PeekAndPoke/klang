...
### Pattern Picking & Selection ✅ (Partial Implementation)

**Basic Picking (innerJoin)** - Select patterns/values from lists or maps by index/key:

- `pick(lookup, pat)` - Pick patterns/values by index (clamps out-of-bounds, uses innerJoin timing)
- `pickmod(lookup, pat)` - Like pick but wraps indices with modulo instead of clamping

**Outer Join Variants** - Preserve outer pattern timing:

- `pickOut(lookup, pat)` - Pick using outer pattern timing (timing from selector, values from picked)
- `pickmodOut(lookup, pat)` - Outer timing with modulo wrapping

**Squeeze/Inhabit** - Compress entire pattern cycles into events:

- `inhabit(lookup, pat)` / `pickSqueeze(lookup, pat)` - Compress picked patterns into selector events
- `inhabitmod(lookup, pat)` / `pickmodSqueeze(lookup, pat)` - Compress with wrapping

All pick functions available as:

- Pattern methods: `"0 1 2 3".pick(["a b", "c d"])`
- String extensions: `"0 1 2".pick(["bd", "sd"])`
- Standalone functions: `pick(["bd", "sd"], "0 1 2")`

**Implementation:**

- **pick/pickmod**: Use `PickInnerPattern` with innerJoin timing - selected pattern plays at its own speed
- **pickOut/pickmodOut**: Use `PickOuterPattern` with outerJoin timing - selector determines event timing
- **inhabit/pickSqueeze**: Use `PickSqueezePattern` - compresses entire pattern cycle into each selector event

**Not Yet Implemented:**

- pickRestart, pickmodRestart (restart variants)
- pickReset, pickmodReset (reset variants)
- squeeze(pat, xs) (direct squeeze function)
...
- `lpf(freq)` - Low-pass filter (aliases: `cutoff`, `ctf`, `lp`)
- `hpf(freq)` - High-pass filter (aliases: `hp`, `hcutoff`)
- `bandf(freq)` / `bpf(freq)` - Band-pass filter (alias: `bp`)
- `notchf(freq)` - Notch filter
- `resonance()` / `res()` / `lpq()` - Low-pass filter resonance/Q factor
- `hresonance()` / `hres()` / `hpq()` - High-pass filter resonance/Q factor
- `bandq()` / `bpq()` - Band-pass filter Q factor
- `nresonance()` / `nres()` - Notch filter resonance/Q factor

### Filter Envelopes ✅

**Lowpass Filter Envelope:**

- `lpattack()` / `lpa()` - LPF envelope attack time
- `lpdecay()` / `lpd()` - LPF envelope decay time
- `lpsustain()` / `lps()` - LPF envelope sustain level
- `lprelease()` / `lpr()` - LPF envelope release time
- `lpenv()` / `lpe()` - LPF envelope modulation depth

**Highpass Filter Envelope:**

- `hpattack()` / `hpa()` - HPF envelope attack time
- `hpdecay()` / `hpd()` - HPF envelope decay time
- `hpsustain()` / `hps()` - HPF envelope sustain level
- `hprelease()` / `hpr()` - HPF envelope release time
- `hpenv()` / `hpe()` - HPF envelope modulation depth

**Bandpass Filter Envelope:**

- `bpattack()` / `bpa()` - BPF envelope attack time
- `bpdecay()` / `bpd()` - BPF envelope decay time
- `bpsustain()` / `bps()` - BPF envelope sustain level
- `bprelease()` / `bpr()` - BPF envelope release time
- `bpenv()` / `bpe()` - BPF envelope modulation depth

**Notch Filter Envelope:** (⚠️ NOT IN ORIGINAL STRUDEL JS)

- `nfattack()` / `nfa()` - Notch filter envelope attack time
- `nfdecay()` / `nfd()` - Notch filter envelope decay time
- `nfsustain()` / `nfs()` - Notch filter envelope sustain level
- `nfrelease()` / `nfr()` - Notch filter envelope release time
- `nfenv()` / `nfe()` - Notch filter envelope modulation depth
...
- `delayfeedback()` / `delayfb()` / `dfb()` - Delay feedback amount

### Distortion & Degradation ✅

- `distort()` / `dist()` - Distortion effect
- `crush()` - Bit crushing
...
- `rangex(min, max)` - Exponential range scaling for unipolar signals (pattern method, string extension)
- `range2(min, max)` - Bipolar range scaling for -1 to 1 signals (pattern method, string extension)
...
- `round()` - Round values to nearest integer (pattern method, string extension)
- `floor()` - Floor values to lower integer (pattern method, string extension)
- `ceil()` - Ceil values to higher integer (pattern method, string extension)
- `ratio()` - Divide numbers via colon notation (e.g., "5:4" = 1.25) (standalone function, pattern method, string
  extension)
...
- 198+ DSL functions/methods (including arithmetic, bitwise, comparison, and logical operators)
...
- 39 continuous oscillators and signal generators (steady, signal, time, sine, sine2, cosine, cosine2, saw, saw2, isaw,
  isaw2, tri, tri2, itri, itri2, square, square2, perlin, perlin2, berlin, berlin2, rand, rand2, irand, randL, randrun,
  brand, brandBy, segment, seg, toBipolar, fromBipolar, rangex, range2, round, floor, ceil, ratio) - many with control
  pattern support
- 4 filter types
...
- ✅ **Euclidean Morph Pattern** - New `euclidish()` / `eish()` function for morphing between Euclidean and evenly
  distributed rhythms
- ✅ **EuclideanMorphPattern** - New pattern type that interpolates pulse positions based on groove parameter
- ✅ **Bjorklund Algorithm** - Extracted to shared `math/bjorklund.kt` for reuse across pattern types
- ✅ **Binary Pattern Functions** - Complete set of binary conversion functions:
...
- ✅ **VoiceValue.Seq Support** - Enhanced GraalVM bridge to handle sequence values for list-based patterns
- ✅ **Comprehensive Testing** - Added test suites for all new features: `LangEuclidishSpec`, `LangRunSpec`,
...
- ✅ **Euclidean Rhythm Functions** (for use outside mini-notation):
...
- ✅ **Pattern Picking Functions (lang_pattern_picking.kt)** - Complete implementation of pattern picking with List and
  Map lookups:

  **Core Pick Functions (innerJoin timing):**
  - `pick(lookup, pat)` - Select patterns/values from List (by index) or Map (by key), clamps indices
  - `pickmod(lookup, pat)` - Like pick but wraps indices with modulo instead of clamping
  - Both use `PickInnerPattern` - selected pattern plays at its own speed
  - All available as pattern extension, standalone function, and string extension

  **Outer Join Pick Functions (outerJoin timing):**
  - `pickOut(lookup, pat)` - Pick using outer pattern timing (selector determines event timing)
  - `pickmodOut(lookup, pat)` - Like pickOut but wraps indices with modulo
  - Both use `PickOuterPattern` - timing from selector, values from picked patterns
  - All available as pattern extension, standalone function, and string extension

  **Squeeze/Inhabit Functions:**
  - `inhabit(lookup, pat)` / `pickSqueeze(lookup, pat)` - Picks and squeezes patterns into selector event timespan
  - `inhabitmod(lookup, pat)` / `pickmodSqueeze(lookup, pat)` - Like inhabit but wraps indices with modulo
  - Both use `PickSqueezePattern` for implementation
  - All available as pattern extension, standalone function, and string extension

  **Implementation Details:**
  - Helper function `reifyLookup()` - Converts Lists and Maps to pattern lookups, wraps non-patterns in `AtomicPattern`
  - Helper function `extractIndex()` - Safely extracts numeric indices with modulo or clamp behavior
  - Helper function `extractKey()` - Safely extracts string keys from values
  - Pattern implementations: `PickInnerPattern`, `PickOuterPattern`, and `PickSqueezePattern`
  - Supports both List-based (integer index) and Map-based (string key) lookups
  - Automatically wraps non-pattern values in `AtomicPattern` with default modifier

  **Testing:**
  - `LangPickInnerSpec.kt` - Tests for pick/pickmod with List and Map lookups
  - `LangPickSqueezeSpec.kt` - Tests for inhabit/inhabitmod behavior

**Recent Improvements (2026-01-16):**

- ✅ **New Pattern Creation Functions:**
...
- ✅ **Control Pattern Support for All Numerical Functions** - Completed major initiative to add control pattern support
...
- ✅ **New Pattern Classes for Control Pattern Support:**
...
- 🎯 **Major milestone**: Most numerical parameters now support control patterns for dynamic behavior
- 🚀 **Performance**: Parser cache significantly reduces overhead for repeated pattern parsing
- 🎨 **Expressiveness**: Users can create much more dynamic and evolving patterns
- 🏗️ **Architecture**: Established clear patterns for implementing control pattern support
