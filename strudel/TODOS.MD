...
- [ ] **`euclid(pulses, steps)`** - Euclidean rhythm function (for use outside mini-notation)
- [ ] **`euclidRot(pulses, steps, rotation)`** - Euclidean with rotation (for use outside mini-notation)
- [ ] **`euclidLegato(pulses, steps)`** - Holds pulses until next (for use outside mini-notation)
...
- [x] **`irand(n)`** - Random integers from 0 to n-1 (returns sequence pattern with n values)
...
- [x] **`seed(n)`** - Set random seed for deterministic pattern generation (works as pattern method and string
  extension)
...
- [x] **`negateValue()`** - Negates the value field of voice data (pattern method, string extension)
...
- **Pattern Creation**: ~7 missing (6 completed: `seq`, `mini`, `stack`, `cat`, `arrange`, `silence`/`rest`)
- **Time Modification**: ~19 missing (4 completed: `fast`, `slow`, `early`, `late`, `gap`)
...
- **Continuous Signals**: ~3 missing (25 completed: `steady`, `signal`, `time`, `sine`, `sine2`, `cosine`, `cosine2`,
  `saw`, `saw2`, `isaw`, `isaw2`, `tri`, `tri2`, `itri`, `itri2`, `square`, `square2`, `perlin`, `berlin`, `range`,
...
- **Conditional & Probabilistic**: ✅ 0 missing (17 completed: `sometimesBy`, `sometimes`, `often`, `rarely`,
  `almostAlways`, `almostNever`, `always`, `never`, `someCyclesBy`, `someCycles`, `degradeBy`, `degrade`, `undegradeBy`,
  `undegrade`, `firstOf`, `every`, `lastOf`)
- **Arithmetic Addons (Non-Strudel)**: 0 missing (2 completed: `negateValue`, `oneMinusValue`)
...
### Total Estimated: ~122 features remaining

### Completed So Far: ~128 features! 🎉
