...
- [ ] **`:n` notation** - Sample number selection in mini-notation (e.g., `"bd:3 hh:2"`)
- [ ] **`-` (dash) rest** - Currently only `~` works for rests

#### Timing Modifiers ⭐⭐⭐

- [ ] **`@n` elongation** - Extend note duration (e.g., `"c@3"` holds for 3 steps)
- [ ] **`!n` replication** - Replicate notes (e.g., `"c!2"` = `"c c"`)

#### Probabilistic ⭐⭐⭐

- [ ] **`?` random removal** - Probabilistic event removal (e.g., `"bd?"` or `"bd?0.5"`)
- [ ] **`|` random choice** - Random selection between alternatives (e.g., `"bd|hh"`)

#### Euclidean Rhythms ⭐⭐⭐

- [ ] **`(pulses, steps)` notation** - Euclidean rhythm in mini-notation (e.g., `"bd(3,8)"`)
- [ ] **`(pulses, steps, rotation)`** - With rotation parameter
...
- [ ] **`rev()`** - Reverses pattern in each cycle
...
- [ ] **`degradeBy(amount)`** - Probabilistic event removal
...
- **Mini-Notation**: 8 features missing
