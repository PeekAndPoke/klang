...
class MidiTest : StringSpec({
    "isMidi" {
        isMidi(100) shouldBe true
        isMidi("100") shouldBe true
        isMidi(0) shouldBe true
        isMidi(127) shouldBe true
        isMidi(-1) shouldBe false
        isMidi(128) shouldBe false
        isMidi("blah") shouldBe false
    }

    "toMidi" {
        toMidi(100) shouldBe 100
        toMidi("C4") shouldBe 60
        toMidi("60") shouldBe 60
        toMidi(0) shouldBe 0
        toMidi("0") shouldBe 0
        toMidi(-1) shouldBe null
        toMidi(128) shouldBe null
        toMidi("blah") shouldBe null
        toMidi(Double.NaN) shouldBe null
    }

    "freqToMidi" {
        freqToMidi(220.0) shouldBe 57.0
        freqToMidi(261.62) shouldBe 60.0
        freqToMidi(261.0) shouldBe 59.96
    }

    "midiToFreq" {
        midiToFreq(60.0) shouldBe 261.6255653005986
        midiToFreq(69.0, 443.0) shouldBe 443.0
    }

    "midiToNoteName" {
        val notes = listOf(60.0, 61.0, 62.0, 63.0, 64.0, 65.0, 66.0, 67.0, 68.0, 69.0, 70.0, 71.0, 72.0)
        notes.map { midiToNoteName(it) }.joinToString(" ") shouldBe "C4 Db4 D4 Eb4 E4 F4 Gb4 G4 Ab4 A4 Bb4 B4 C5"
        notes.map { midiToNoteName(it, sharps = true) }
            .joinToString(" ") shouldBe "C4 C#4 D4 D#4 E4 F4 F#4 G4 G#4 A4 A#4 B4 C5"
        notes.map { midiToNoteName(it, pitchClass = true) }.joinToString(" ") shouldBe "C Db D Eb E F Gb G Ab A Bb B C"

        midiToNoteName(Double.NaN) shouldBe ""
        midiToNoteName(Double.NEGATIVE_INFINITY) shouldBe ""
        midiToNoteName(Double.POSITIVE_INFINITY) shouldBe ""
    }
    ...
    "pcsetNearest find nearest upwards" {
        val nearest = pcsetNearest(listOf(0, 5, 7))
    ...
    "pcsetNearest chromatic to nearest C minor pentatonic" {
        val nearest = pcsetNearest("100101010010")
        listOf(
            36.0,
            37.0,
            38.0,
    ...
    "pcsetNearest chromatic to nearest half octave" {
        val nearest = pcsetNearest("100000100000")
        listOf(
            36.0,
            37.0,
            38.0,
    ...
    "pcsetNearest empty pcsets returns null" {
        listOf(10.0, 30.0, 40.0).map { pcsetNearest(emptyList<Int>())(it) } shouldBe listOf(null, null, null)
    }

    "pcsetSteps" {
        val scale = pcsetSteps("101010", 60.0)
    ...
    "pcsetDegrees" {
        val scale = pcsetDegrees("101010", 60.0)
