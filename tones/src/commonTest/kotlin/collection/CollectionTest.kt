...
class CollectionTest : StringSpec({
    ...
    "rotate" {
        rotate(2, "a b c d e".split(" ")) shouldBe "c d e a b".split(" ")
    }

    "compact" {
        val input = listOf("a", 1, 0, true, null, null)
        val result = listOf("a", 1, 0, true)
        compact(input) shouldBe result
    }

    "shuffle" {
        val rnd = { 0.2 }
        shuffle("a b c d".split(" "), rnd) shouldBe listOf("b", "c", "d", "a")
    }

    "permutations" {
        permutations(listOf("a", "b", "c")) shouldBe listOf(
            listOf("a", "b", "c"),
            listOf("b", "a", "c"),
            listOf("b", "c", "a"),
            listOf("a", "c", "b"),
