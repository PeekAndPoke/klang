 ...
 */
fun detectChord(source: List<String>, options: DetectOptions = DetectOptions()): List<String> {
    val notes = source.map { note(it).pc }.filter { it.isNotEmpty() }
    if (notes.isEmpty()) {
        return emptyList()
    }
