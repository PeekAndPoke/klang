...
import io.peekandpoke.klang.tones.collection.compact
import io.peekandpoke.klang.tones.midi.midiToNoteName
import io.peekandpoke.klang.tones.midi.toMidi
import io.peekandpoke.klang.tones.collection.range as rangeInt
 ...
 */
fun numericRange(notes: List<Any>): List<Int> {
    val midi: List<Int> = compact(
        notes.map { note ->
            when (note) {
    ...
    for (i in 1 until midi.size) {
        ...
        val next = midi[i]
        result.addAll(rangeInt(last, next).drop(1))
    }
...
fun chromaticRange(
    ...
    return numericRange(notes).map { midi ->
        midiToNoteName(midi.toDouble(), sharps = sharps, pitchClass = pitchClass)
    }
}
