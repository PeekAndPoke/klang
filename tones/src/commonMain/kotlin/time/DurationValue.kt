...
private val VALUES: List<DurationValue> = DATA.map { (denominator, shorthand, names) ->
    DurationValue(
        ...
        fraction = if (denominator < 1.0) Pair((1.0 / denominator).toInt(), 1) else Pair(1, denominator.toInt()),
        shorthand = shorthand,
        names = names
    )
}
 ...
 */
fun durationNames(): List<String> = VALUES.flatMap { it.names }

/**
 * Returns a list of all duration shorthands.
 */
fun durationShorthands(): List<String> = VALUES.map { it.shorthand }
 ...
 */
fun getDurationValue(name: String): DurationValue {
    val match = REGEX.matchEntire(name) ?: return DurationValue.NoDuration
    val simple = match.groupValues[1]
    val dots = match.groupValues[2]

    val base = VALUES.find { it.shorthand == simple || simple in it.names }
        ?: return DurationValue.NoDuration
