 ...
 *   host call (incl. presets) and commits the full argument list. voices commits as an
...
private class SprudelSuperPluckEditorComp(ctx: Ctx<Props>) : Component<SprudelSuperPluckEditorComp.Props>(ctx) {
    ...
    private val parsedBrightness
        get() = parseNum(call?.args?.getOrNull(3), 0.5)

    private val parsedPickPosition
        get() = parseNum(call?.args?.getOrNull(4), 0.5)

    private val parsedStiffness
        get() = parseNum(call?.args?.getOrNull(5), 0.0)
    ...
    private val parseable: List<Boolean> = List(6) { parseNumOrNull(call?.args?.getOrNull(it)) != null }
    ...
    private fun FlowContent.renderContent() {
        div {
            ...
            // Presets set all six params, so they only make sense in whole-call mode
            if (call != null) {
                div {
            ...
            ui.form {
                ...
                // Row 2: String params (whole-call mode only)
                if (call != null) {
                    ui.four.stackable.fields {
                        ...
                        UiInputField(pickPosition, { pickPosition = it; dirty += 4; liveUpdate() }) {
                            domKey("pickPosition")
                            step(0.05)
                            label {
                                +"Pick Pos"
                        ...
                        UiInputField(stiffness, { stiffness = it; dirty += 5; liveUpdate() }) {
                            domKey("stiffness")
                            step(0.05)
                            label {
                                +"Stiffness"
