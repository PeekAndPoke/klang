...
private fun Tag.SprudelGainEditorComp(toolCtx: KlangUiToolContext, embedded: Boolean) =
    comp(SprudelGainEditorComp.Props(toolCtx, embedded)) { SprudelGainEditorComp(it) }

// ── Component ─────────────────────────────────────────────────────────────────

private class SprudelGainEditorComp(ctx: Ctx<Props>) : Component<SprudelGainEditorComp.Props>(ctx) {
    ...
    override fun VDom.render() {
        if (props.embedded) {
            ...
            ui.segment {
                css { minWidth = 400.px }
                toolHeaderWithInfo("Gain", props.toolCtx, infoPopup)
                renderContent()
                ui.divider {}
                ToolButtonBar(
