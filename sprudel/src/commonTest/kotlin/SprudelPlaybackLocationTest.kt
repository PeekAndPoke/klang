...
/**
 * Tests for source location tracking through the SprudelPlayback event callback
 */
class SprudelPlaybackLocationTest : StringSpec({

    "SprudelPlayback callback receives events with sourceLocations" {
        val baseLocation =
        ...
        val pattern = parseMiniNotation("bd", baseLocation) { text, sourceLocations ->
            AtomicPattern(SprudelVoiceData.empty.copy(note = text), sourceLocations)
        }
