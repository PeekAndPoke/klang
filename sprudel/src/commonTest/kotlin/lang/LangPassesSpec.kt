 ...
 * `passes` slots, on both doors; absent = 1 (bit-identical single stage); non-positive
...
class LangPassesSpec : StringSpec({
    ...
    "the passes slot alone sets the count without touching the head fields" {
        val lp = firstLp(note("c").lpf(freq = 800, passes = 2))
        lp.passes shouldBe 2
        lp.freq shouldBe 800.0
        val viaScript = firstHp(SprudelPattern.compile("""note("c").hpf(freq = 200, passes = 2)"""))
    ...
    "non-positive counts coerce to 1 at the wire boundary — no throw" {
        firstLp(note("c").lpf(freq = 800, passes = 0)).passes shouldBe 1
        firstLp(note("c").lpf(freq = 800, passes = -3)).passes shouldBe 1
    }

    "an absurd count coerces to the resource ceiling — no throw, no billion-stage allocation" {
        firstLp(note("c").lpf(freq = 800, passes = 1_000_000)).passes shouldBe FILTER_MAX_PASSES
        firstHp(note("c").hpf(freq = 200, passes = 1e9)).passes shouldBe FILTER_MAX_PASSES
    }
    ...
    "fractional counts ROUND, they do not truncate (and NaN falls back to 1)" {
        ...
        firstLp(note("c").lpf(freq = 800, passes = 2.7)).passes shouldBe 3
        firstLp(note("c").lpf(freq = 800, passes = 2.4)).passes shouldBe 2
        firstLp(note("c").lpf(freq = 800, passes = 2.9999999996)).passes shouldBe 3
        // roundToInt() THROWS on NaN, and this runs on the render thread.
        firstHp(note("c").hpf(freq = 200, passes = Double.NaN)).passes shouldBe 1
    }

    "a tail-only head call must NOT reinterpret the pattern's values as cutoff" {
        // The head applies only on a bare call or when `freq` is given: any of the seven tail slots alone leaves it untouched. Relaxing it
    ...
    "mapper forms (c) and (d): the standalone and the CHAINED mapper both carry passes" {
        ...
        firstLp(note("c").apply(lpf(800).lpf(passes = 3))).passes shouldBe 3
        firstHp(note("c").apply(hpf(200, 1.0, 2))).passes shouldBe 2
        firstHp(note("c").apply(hpf(200).hpf(passes = 3))).passes shouldBe 3
    }
})
