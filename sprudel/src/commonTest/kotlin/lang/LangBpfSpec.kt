...
class LangBpfSpec : StringSpec({
    ...
    "reinterpret voice data as bandf | \"1000 500\".bpf()" {
        val p = "1000 500".bpf()
        val events = p.queryArc(0.0, 1.0)
        assertSoftly {
            events.size shouldBe 2
    ...
    "reinterpret voice data as bandf | seq(\"1000 500\").apply(bpf())" {
        val p = seq("1000 500").apply(bpf())
        val events = p.queryArc(0.0, 1.0)
        assertSoftly {
            events.size shouldBe 2
            events[0].data.bandf shouldBe 1000.0
    ...
    "bpf() works as pattern extension" {
        val p = note("c").bpf("1000")
        val events = p.queryArc(0.0, 1.0)
    ...
    "bpf() works in compiled code" {
        val p = SprudelPattern.compile("""note("c").bpf("1000")""")
