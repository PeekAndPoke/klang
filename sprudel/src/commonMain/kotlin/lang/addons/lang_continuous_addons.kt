 ...
 * sound("sd").delay(wet = 0.25, time = pure(1/8).div(cps), feedback = 0.5)  // Dalay time based in CPS
 * ```
 *
 * @category continuous
 ...
 *
 * ```KlangScript(Playable)
 * sound("sd").delay(wet = 0.15, time = pure(60).div(bpm), feedback = 0.33)  // Dalay time based in BPM
 * ```
 *
 * @category continuous
