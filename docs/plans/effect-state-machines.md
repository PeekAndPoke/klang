...
| `ResonatorBank` (the body and vowel morph) | A third commit on body and vowel, Katalyst 5c-10 (2026-09-20), adds NO state: the morph lives in the bank and the swap only answers `isTarget`. The bank's own lifecycle is again `KnobGlide`'s shape (a snap flag, a countdown, a rest), the same argument the gain, phaser and duck rows make; its four questions are answered in its KDoc | the swap's output crossfade, kept for the edges |
