...
| W4 | `coarse` NaN amount LATCHES both doors permanently (NaN fails `<= 1.0`, poisons `counter`, both refresh predicates false forever -> frozen DC that never heals; `+Inf` behaves differently — infinite hold that DOES heal); ignitor door also lacks the strip's `nanGuard()` on the captured sample (a NaN input holds for up to `amount` samples there, zero on the strip). Low reach (arithmetic is safeOut-clamped; needs a NaN oscp override or raw Kotlin) | note | **SPLIT**: ignitor half DONE via W3's non-finite arm (heals, guarded); STRIP half OPEN — `CoarseRenderer` still latches permanently on a NaN/Inf constructor amount (documented at the class; reach = a NaN pattern value) |
