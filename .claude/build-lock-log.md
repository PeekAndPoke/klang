# Build-lock log: what past sessions left behind

Archive of the handover notes that used to sit in `.claude/BUILD-LOCK.md`. **You do not read this
file to take the lock.** Read it only when you are already looking for something specific: what a
named session touched, why an uncommitted change is in the tree, what a note meant.

Newest first. Nothing here is authoritative, the commit history is. An entry survives only as the
sentence a commit message did not have room for.

**Appending:** when you release the lock, move your handover note here, above the previous one.
The live file keeps a row only while your work is uncommitted.

---

> Last action (2026-08-31, claude-code tutorial-curriculum session): A7 "Space and Dirt"
> authored (`tut_SpaceAndDirt.kt`; roomWet/rsize/delayWet/delaytime/delayfb/distort/onepole/
> postgain). Green: `:jvmTest --tests io.peekandpoke.klang.TutorialCurriculumSpec` 14/14 with
> 17 lessons registered. Four more sprudel KDoc corrections (three `Playable` reverb/delay
> examples were SILENT because the send's gate parameter was missing; `postgain`'s "applied
> before synthesis" was wrong, both land in SendRenderer). UNCOMMITTED.

> Earlier this session: A6 "Thickness" authored
> (`tut_Thickness.kt`, unison/spread/analog) + two corpus-wide style sweeps (bold **Update**,
> comment-column alignment) now guarded by checks 10 and 11 in `TutorialCurriculumSpec`.
> Green: `:jvmTest --tests io.peekandpoke.klang.TutorialCurriculumSpec` 14/14, and both new
> checks were mutation-checked (each bites, each restored to green). Touched sprudel KDoc
> only (`analog` unit was documented as 0..1, it is CENTS; eight `unison` examples called a
> `.detune()` that no longer exists), so sprudel KSP re-ran once. UNCOMMITTED.

> Last action (2026-08-31, claude-code audio-audit session): **F7's zero-coverage half CLOSED, and
> it found a live bug on its first run.** New `VoiceSchedulerSoloCutSpec` (11 rows) is the first spec
> to take `VoiceScheduler`'s own logic as its subject: 6 rows on the solo duck, 4 on cut/choke, 1
> deliberate positive control with no mutation of its own (so a wrong pan convention or a 0 Hz
> oscillator cannot make the other ten vacuous). **7 mutations, 7 killed.**
> **NEW FINDING F18, FIXED (one argument):** `VoiceFactory.buildVoice` takes `cut: Int? = null` and
> of its two call sites only the SAMPLE branch passed it — so cut/choke groups were **silently inert
> on every synth voice**, which could still choke sample voices while being immune to being choked
> themselves (trigger reads `data.cut`, victim reads `voice.cut`). That asymmetry is what settles it
> as a slip, not a design. Zero shipped songs use `.cut(`, so NO SOUND MOVES.
> Also this session: re-verification sweep over all 10 open findings. **Five were wrong** —
> F14's Pedal bullet, three of F15(a)'s "defects" (they assert via custom infix matchers this repo
> uses: `intShouldBeLessThan`, `should beGreaterThanOrEqualTo`), and F7's own "not reached by any
> spec" (`SampleVoiceOnsetSpec` + `SeededPlaybackReproducibilitySpec` both drive it through
> `engine.scheduler` and never name the type). Every one of those was a grep standing in for reading
> the code — including my own first re-verification pass. Ledger corrected in docs/audio-audit/.
> Green: `:audio_be:jvmTest`, `:audio_bridge:jvmTest`, `:audio_be:compileKotlinJs`. UNCOMMITTED.
> ✅ The F3/F4/F8 mutations are now RE-RUN, against the whole 1373-test suite (not just their own
> spec, which is what the pilot did wrong): F3 and F8 SURVIVE and stand; **F4 is KILLED by
> `PipelinePresetSpec` and is WITHDRAWN** — wrong when written, same file-scoped-claim-stated-as-
> suite-wide error as F14's Pedal bullet, and the same spec falsifies both. Standing rule added to
> the INDEX: a mutation verdict is only as wide as the test selection it ran against.
> Also filed unfixed: **F19** — `cut(0)` is documented as "no choke" but the engine gates on
> `cut != null`, so group 0 chokes group 0. Needs a maintainer call (fix the engine, or the doc).
> Working rule for the rest of the campaign is still ANALYSIS ONLY, triage as a batch.
> ⚠️ Three workstreams share this tree right now — ignitor-arithmetic (uncommitted `windowEnd`) and
> a tutorials/sprudel one. Mine are ONLY: VoiceFactory.kt, VoiceSchedulerSoloCutSpec.kt,
> docs/audio-audit/*.
...
*The two sections below predate the log convention: long "what the last holder changed" writeups
from 2026-08. Both are committed work, and both rules they state now live in the code itself
(`BackendClock.kt`, `BlockContext.kt`, `constants/MasterLimiterDefaults.kt`).*
