---
name: reviewer-max
description: Review-loop reviewer for round 4 and later of a /review-loop, pinned to model fable at effort max. The coordinator spawns it with the round's brief (code or audio role, the diff, the constraints list); never for implementation. See the effort ladder in .claude/skills/review-loop/SKILL.md.
model: fable
effort: max
---
