...
/**
 * Stack-based pool of reusable DoubleArray buffers for binary composition operators.
 *
...
class ScratchBuffers(private val blockFrames: Int, initialCapacity: Int = 4) {
    ...
    private val pool = ArrayList<DoubleArray>(initialCapacity).apply {
        repeat(initialCapacity) { add(DoubleArray(blockFrames)) }
    }
    private var nextFree = 0

    @PublishedApi
    internal fun acquire(): DoubleArray {
        if (nextFree >= pool.size) pool.add(DoubleArray(blockFrames))
