...
import io.peekandpoke.klang.audio_be.orbits.bus.*
...
class Orbit(val id: Int, val blockFrames: Int, sampleRate: Int, private val silentBlocksBeforeTailCheck: Int = 10) {
    ...
    val delay = BusDelayEffect(
        delayLine = DelayLine(maxDelaySeconds = 10.0, sampleRate = sampleRate),
    )

    val reverb = BusReverbEffect(
        reverb = Reverb(sampleRate),
    )

    val phaser = BusPhaserEffect(
        phaser = Phaser(sampleRate),
    )

    val compressor = BusCompressorEffect()

    val ducking = BusDuckingEffect()

    /**
     * The bus effect pipeline: Delay → Reverb → Phaser → Compressor.
     *
     * Ducking is NOT in this pipeline — it's applied separately by [Orbits] after all orbits
     * are processed, because it needs cross-orbit access to the sidechain source.
     */
    val pipeline: List<BusEffect> = listOf(delay, reverb, phaser, compressor)
    ...
    fun updateFromVoice(voice: Voice) {
        ...
        if (voiceDucking != null) {
            ducking.duckOrbitId = voiceDucking.orbitId
     ...
     *
     * Ducking is NOT processed here — see [Orbits.processAndMix].
     */
    fun processEffects() {
        if (!isActive) return

        for (effect in pipeline) {
            effect.process(busContext)
        }
    }
     ...
     *
     * Called by [Orbits] after all orbits have processed their main pipeline,
     * since ducking needs cross-orbit access.
     */
    fun processDucking(sidechainMixBuffer: StereoBuffer?) {
        if (!isActive) return

        busContext.sidechainBuffer = sidechainMixBuffer
        ducking.process(busContext)
        busContext.sidechainBuffer = null
    }
