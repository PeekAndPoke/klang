...
class BusPipelineSpec : StringSpec({
    ...
    fun createPipeline(
        ...
        val delay = BusDelayEffect(DelayLine(10.0, sampleRate).apply { delayTimeSeconds = delayTime })
        val reverb = BusReverbEffect(Reverb(sampleRate).apply { roomSize = reverbRoom })
        val phaser = BusPhaserEffect(Phaser(sampleRate).apply {
            depth = phaserDepth
            rate = 2.0
            centerFreq = 1000.0
            sweepRange = 1000.0
        ...
        val compressor = BusCompressorEffect().apply {
