 ...
 *
 * @param _xs   sorted array of x-values (size = N+1)
 * @param _pdf  Probability density values
 */
@SerialName("bucketed")
data class BucketedDistribution(val xs: List<Double>, val pdf: List<Double>) : Distribution {

    val _xs: DoubleArray = xs.toDoubleArray()
    val _pdf: DoubleArray = pdf.toDoubleArray()
    ...
    fun inverse(): BucketedDistribution {
        // Safely grab the extremes
        val pMax = _pdf.maxOrNull() ?: error("pdf must not be empty")
        val pMin = _pdf.minOrNull() ?: error("pdf must not be empty")
    ...
    fun reversed(): BucketedDistribution {
        // leave xs ascending
        val revPdf = pdf.reversed()
        return BucketedDistribution(xs = xs, pdf = revPdf)
    }
    ...
    /**
     * 1) Compute the CDF via the trapezoid rule.
     */
    fun computeCdf(xs: DoubleArray, pdf: DoubleArray): DoubleArray {
        val n = xs.size
