...
class KlangScriptSuperSquareSpec : StringSpec({
    ...
    "phasePool(1)" {
        ks("Osc.supersquare().phasePool(1)") shouldBe superSquare().copy(phasePool = 1.0)
    }

    "drawTries(8)" {
        ks("Osc.supersquare().drawTries(8)") shouldBe superSquare().copy(drawTries = 8.0)
    }

    "kMin(0.2)" {
        ks("Osc.supersquare().kMin(0.2)") shouldBe superSquare().copy(kMin = 0.2)
    }

    "kMax(0.7)" {
        ks("Osc.supersquare().kMax(0.7)") shouldBe superSquare().copy(kMax = 0.7)
    }

    "poolSize(64)" {
        ks("Osc.supersquare().poolSize(64)") shouldBe superSquare().copy(poolSize = 64.0)
    }

    "refreshEvery(5)" {
        ks("Osc.supersquare().refreshEvery(5)") shouldBe superSquare().copy(refreshEvery = 5.0)
    }

    "selection(1)" {
        ks("Osc.supersquare().selection(1)") shouldBe superSquare().copy(selection = 1.0)
    }
