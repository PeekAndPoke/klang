...
class KlangScriptSuperTriSpec : StringSpec({
    ...
    "poolSize(64)" {
        ks("Osc.supertri().poolSize(64)") shouldBe superTri().copy(poolSize = 64.0)
    }

    "refreshEvery(5)" {
        ks("Osc.supertri().refreshEvery(5)") shouldBe superTri().copy(refreshEvery = 5.0)
    }

    "selection(1)" {
        ks("Osc.supertri().selection(1)") shouldBe superTri().copy(selection = 1.0)
    }
