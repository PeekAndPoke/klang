...
class IgnitorDslSerializationTest : StringSpec({
    ...
    "Variants round-trips with multiple primitive children" {
    ...
    "Variants round-trips with composed child expressions" {
        val dsl = IgnitorDsl.Variants(
            listOf(
                IgnitorDsl.Sine().lowpass(2000.0),
                IgnitorDsl.Sawtooth().drive(0.4),
            )
        )
        val result = roundTrip(dsl)
        result shouldBe dsl
        result.shouldBeInstanceOf<IgnitorDsl.Variants>()
        result.children.size shouldBe 2
    }

    "Variants round-trips when nested inside another Variants" {
        val dsl = IgnitorDsl.Variants(
            listOf(
