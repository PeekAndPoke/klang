 ...
 *
 * ## Why this lives in `audio_bridge` (moved 2026-08-29)
 *
 * Nearly every constant here is a **wire default**: it is the default of a field on an `IgnitorDsl`
 * node, which crosses the wire. While these were `internal` in `audio_be`, the DSL could not see
 * them, so the same numbers were hand-copied into `IgnitorDsl` and guarded by sync specs. That
 * duplication drifted the moment someone retuned by ear: the saw flyback went 2 → 1 sample here,
 * the hand-copy did not, and `supersaw` got brighter while a plain `saw` stayed dull. Same failure
 * the filter constants had (see `FilterHumanizationDefaults`). Now the DSL references these
 * directly and there is nothing left to drift.
 *
 * **Deliberate exceptions.** Four constants here are engine-internal DSP coefficients with no DSL
 * field, so by the strict rule ("a constant belongs in `audio_bridge` iff it is a wire default")
 * they would stay in `audio_be`: [CRACKLE_CHAOS_MAX], [CRACKLE_C], [CRACKLE_DC_POLE] and
 * [NOISE_TILT_LP_COEF]. They are kept here anyway because this file's whole purpose is to be the
 * ONE place the voice is dialed in by ear — splitting four numbers out would cost more than the
 * layering purity is worth. Do not read them as a licence to add unrelated engine internals here.
 */
...
const val RAMP_RESET_SAMPLES: Double = SAW_RESET_SAMPLES

/** Ramp max flyback fraction of a cycle. Starts equal to the saw. */
const val RAMP_SHAPE_MAX: Double = SAW_SHAPE_MAX

// ── Super-saw (unison) ───────────────────────────────────────────────────────────────────────────

/** Center-dominant gain falloff: 0 = all voices equal (flat), 1 = only the center voice. */
const val SUPERSAW_SIDE_ATTEN: Double = 0.1
 ...
 */
const val SUPERSAW_GAIN_JITTER: Double = 0.15
 ...
 */
const val SUPERSAW_CENTER_JITTER_SCALE: Double = 0.4

/** Detune spacing shape: 1.0 = even; >1 concentrates voices toward center; <1 spreads outward. */
const val SUPERSAW_SPREAD_POWER: Double = 1.2
...
const val SUPERRAMP_SIDE_ATTEN: Double = SUPERSAW_SIDE_ATTEN

/** Super-ramp per-voice amplitude jitter. Starts equal to the super-saw. */
const val SUPERRAMP_GAIN_JITTER: Double = SUPERSAW_GAIN_JITTER

/** Super-ramp detune spacing shape. Starts equal to the super-saw. */
const val SUPERRAMP_SPREAD_POWER: Double = SUPERSAW_SPREAD_POWER

/** Super-ramp center-voice jitter scale. Starts equal to the super-saw. */
const val SUPERRAMP_CENTER_JITTER_SCALE: Double = SUPERSAW_CENTER_JITTER_SCALE

/** Super-ramp phase pool knobs. Start equal to the super-saw (same harmonic statistics). */
const val SUPERRAMP_PHASE_POOL: Double = SUPERSAW_PHASE_POOL
const val SUPERRAMP_DRAW_TRIES: Double = SUPERSAW_DRAW_TRIES
const val SUPERRAMP_K_MIN: Double = SUPERSAW_K_MIN
const val SUPERRAMP_K_MAX: Double = SUPERSAW_K_MAX
const val SUPERRAMP_POOL_SIZE: Double = SUPERSAW_POOL_SIZE
const val SUPERRAMP_REFRESH_EVERY: Double = SUPERSAW_REFRESH_EVERY
const val SUPERRAMP_SELECTION: String = SUPERSAW_SELECTION
const val SUPERRAMP_WARMUP: Double = SUPERSAW_WARMUP
...
const val SUPERSQUARE_SIDE_ATTEN: Double = SUPERSAW_SIDE_ATTEN

/** Super-square per-voice amplitude jitter. Starts equal to the super-saw. */
const val SUPERSQUARE_GAIN_JITTER: Double = SUPERSAW_GAIN_JITTER

/** Super-square detune spacing shape. Starts equal to the super-saw. */
const val SUPERSQUARE_SPREAD_POWER: Double = SUPERSAW_SPREAD_POWER

/** Super-square center-voice jitter scale. Starts equal to the super-saw. */
const val SUPERSQUARE_CENTER_JITTER_SCALE: Double = SUPERSAW_CENTER_JITTER_SCALE

/** Super-square phase pool knobs. Start equal to the super-saw (same odd-harmonic statistics). */
const val SUPERSQUARE_PHASE_POOL: Double = SUPERSAW_PHASE_POOL
const val SUPERSQUARE_DRAW_TRIES: Double = SUPERSAW_DRAW_TRIES
const val SUPERSQUARE_K_MIN: Double = SUPERSAW_K_MIN
const val SUPERSQUARE_K_MAX: Double = SUPERSAW_K_MAX
const val SUPERSQUARE_POOL_SIZE: Double = SUPERSAW_POOL_SIZE
const val SUPERSQUARE_REFRESH_EVERY: Double = SUPERSAW_REFRESH_EVERY
const val SUPERSQUARE_SELECTION: String = SUPERSAW_SELECTION
const val SUPERSQUARE_WARMUP: Double = SUPERSAW_WARMUP
...
const val SUPERTRI_SIDE_ATTEN: Double = SUPERSAW_SIDE_ATTEN

/** Super-triangle per-voice amplitude jitter. Starts equal to the super-saw. */
const val SUPERTRI_GAIN_JITTER: Double = SUPERSAW_GAIN_JITTER

/** Super-triangle detune spacing shape. Starts equal to the super-saw. */
const val SUPERTRI_SPREAD_POWER: Double = SUPERSAW_SPREAD_POWER

/** Super-triangle center-voice jitter scale. Starts equal to the super-saw. */
const val SUPERTRI_CENTER_JITTER_SCALE: Double = SUPERSAW_CENTER_JITTER_SCALE
...
const val SUPERTRI_PHASE_POOL: Double = SUPERSAW_PHASE_POOL
const val SUPERTRI_DRAW_TRIES: Double = 16.0
const val SUPERTRI_K_MIN: Double = 0.40
const val SUPERTRI_K_MAX: Double = 0.65
const val SUPERTRI_POOL_SIZE: Double = SUPERSAW_POOL_SIZE
const val SUPERTRI_REFRESH_EVERY: Double = SUPERSAW_REFRESH_EVERY
const val SUPERTRI_SELECTION: String = SUPERSAW_SELECTION
const val SUPERTRI_WARMUP: Double = SUPERSAW_WARMUP
...
const val SUPERSINE_SIDE_ATTEN: Double = SUPERSAW_SIDE_ATTEN

/** Super-sine per-voice amplitude jitter. Starts equal to the super-saw. */
const val SUPERSINE_GAIN_JITTER: Double = SUPERSAW_GAIN_JITTER

/** Super-sine detune spacing shape. Starts equal to the super-saw. */
const val SUPERSINE_SPREAD_POWER: Double = SUPERSAW_SPREAD_POWER

/** Super-sine center-voice jitter scale. Starts equal to the super-saw. */
const val SUPERSINE_CENTER_JITTER_SCALE: Double = SUPERSAW_CENTER_JITTER_SCALE
...
const val SUPERSINE_PHASE_POOL: Double = SUPERSAW_PHASE_POOL
const val SUPERSINE_DRAW_TRIES: Double = 40.0
const val SUPERSINE_K_MIN: Double = 0.50
const val SUPERSINE_K_MAX: Double = 0.80
const val SUPERSINE_POOL_SIZE: Double = SUPERSAW_POOL_SIZE
const val SUPERSINE_REFRESH_EVERY: Double = SUPERSAW_REFRESH_EVERY
const val SUPERSINE_SELECTION: String = SUPERSAW_SELECTION
