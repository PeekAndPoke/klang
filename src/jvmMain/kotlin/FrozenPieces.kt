...
/**
 * FROZEN snapshots of Der Schmetterling for the ledger suite (`runSongBenchmark --args=ledger`):
 * the instrument pieces of the series' posts, held still so an engine change is the only thing
 * that can move their numbers. Captured verbatim from the COMMITTED `builtinsongs/DerSchmetterling.kt`
 * at the tag named in each constant, never from a working tree.
 *
 * Immutable except for door renames: when a door is renamed on both surfaces, a frozen text that
...
object FrozenPieces {
    ...
    /** The song at `v0.3.14` (2026-09-16), the state the September optimization round was measured on. */
