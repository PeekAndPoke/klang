...
stack(
  ...
  morse("Gitarre!").n("0").scale("c4:chromatic").fast(2)
    .gain(1.0).distort(0.5).postgain("0.5!4 1.2!2 1.7 2.0".slow(64)).hpf(4350).lpf(4450) //.solo()
  ,// Melody 1 ---------------------------------------------------------------------------------
  n(`<[0 0 0 7] [0 5 0 2] [0 3 0 5] [0 3 0 0]  [ 0 0 0 7] [0  5 0 8] [0 7 0 5] [ 0 7 0 0]
    ...
    .fast(4).scale("C3:chromatic").clip(0.75).hpf(750).lpf(4500).pan(0.2)
    .s("supersaw").unison(8).detune(0.05).distort(saw.range(1.2, 2.0).slow(32)).gain(0.135) //.solo()
    .adsr("0.02:0.2:0.5:0.05")
  , // Melody 2 --------------------------------------------------------------------------------------------------
  n(`<[0 0 0 7] [0 5 0 2] [0 3 0 5] [0 3 0 0]  [ 0 0 0 7] [0  5 0 8] [0 7 0 5] [ 0 7 0 0]
      [0 0 0 7] [0 5 0 2] [0 3 0 5] [0 3 0 0]  [12@2 0 0] [0 10 0 7] [0 8 7 8] [10 8 7@2]>`)
    .fast(4).scale("C4:chromatic").clip(0.75).hpf(800).lpf(4700).pan(0.8)
    .s("supersaw").unison(8).detune(0.05).distort(saw.range(1.2, 2.0).slow(32)).gain(0.135) //.solo()
    .adsr("0.02:0.2:0.5:0.05")
  , // Rhythm -----------------------------------------------------------------------------------------------------------------
  cat(
    ...
    .s("supersaw").unison(4).detune(0.10).gain(0.5).distort(2.5).postgain(0.235)
    .superimpose(x => x.pan(0.35).bandf("750 1200 900 1400".slow(64)).bandq(saw.range(2.0, 5.0).slow(64)).postgain(0.2))    
    .adsr("0.02:0.2:0.5:0.0").clip(1.01) //.solo()
  , // Noise --------------------------------------------------------------------------------------------------------------
  s("cp cp cp cp").bandf("3200 600 2400 600").gain(saw.range(0.0, 0.4).slow(32)) //.solo()
  ...
  ).adsr("0.01:0.3:1.0:1.0").gain(0.70).hpf(80)  //.solo()
  , // Drums 1 -----------------------------------------------------------------------------------------------
