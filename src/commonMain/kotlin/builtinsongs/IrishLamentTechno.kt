...
let bass = note("<[a1!8] [d2!8] [bb1!8] [c2!8] [g1!8] [f1!8] [a1!8] [d2!8]>")
    ...
    .gain(1.0).orbit(2) // . solo()
let core = stack(kick, hat, bass)

// ── Build layers ────────────────────────────────────────────────────
let sub  = note("<a1 d2 bb1 c2 g1 f1 a1 d2>").struct("x!2").sound("sine").legato(1.0).adsr("0.005:0.05:0.5:0.02").hpf(70).lpf(180).gain(0.45).orbit(3)
...
let leadStyle = mel =>mel.sound("supertri").unison(3).spread(0.07).euclid(3, 8).warmth(0.02)
       ...
       .gain(0.85).orbit(4).room(0.2).rsize(3)
...
let pad = chord("<Am Dm Bb C Gm F Am Dm>").voicing(rank = sine.range(0, 1.9).fast(7).add(perlin.range(0, 0.3)))
    .sound("superpulse").unison(3).spread(0.20).lpf("1800:1:2.5").adsr("1.5:0.5:0.3:1.0").legato(1.4)
    ...
    .gain(0.055).orbit(5).room(0.4).rsize(6) //  .solo()
...
let hitSub  = note("d1").sound("sine")
    ...
    .orbit(1)
let hitCrash = s("cr").gain(0.75).hpf(200).orbit(2).room(0.25).rsize(4).adsr("0.005:0.3:1.0:2.0")
...
let mel3 = melody3a.sound("saw").legato(0.75).hpf(400).lpf(3500).adsr("0.35:0.1:0.6:0.85").gain(0.8).orbit(1).pan(0.3).coarse(3).coarseos(4)
let mel1 = melody1.sound("saw").legato(0.75).hpf(400).lpf(3500).adsr("0.35:0.1:0.6:0.85").gain(0.85).orbit(1).pan(0.7).coarse(3).coarseos(4)
...
let quietBuild = stack(
    ...
    note("<a1 d2 bb1 c2 g1 f1 a1 d2>").struct("<[x]!32 [x!2]!32>").sound("sine").legato(1.0).adsr("0.005:0.05:0.5:0.05")
        .hpf(60).lpf(220).lpenv(4).gain(saw.range(0.2, 0.45).slow(64)).orbit(3),
    ...
    mel3.velocity(saw.range(0.3, 0.9).min(0).max(1).slow(64)).euclidrot(3, 8, 1).vib(4).vibmod(0.05),
    // Melody 1 — velocity fades from silent to full
    mel1.velocity(saw.range(-0.1, 0.9).min(0).max(1).slow(64)).euclidrot(3, 8, 1).vib(4).vibmod(0.10),
    ...
    chord("<Am Dm Bb C Gm F Am Dm>").voicing().struct("[x@3 x@3 x@4 x@2 x@2 x@2]")
        ...
        .gain(0.19).orbit(5).room(0.4).rsize(6)
    ...
    note(`<[a1 e2 a2 e2 c2 e2 a1 e2] [d2 a2 d3 a2 f2 a2 d2 a2]
        ...
        .spread(sine.range(0.15, 0.40).slow(64)).hpf(200).lpf(4800).lpq(1.2)
...
let darkBuild = stack(
    ...
    // Hat
    s("hh!8").gain(0.4).hpf(6000).adsr("0.001:0.04:0.0:0.04").orbit(2),
    // Clap
    ...
    note(`<[a1 e2 a2 e2 c2 e2 a1 e2] [d2 a2 d3 a2 f2 a2 d2 a2] [bb1 f2 bb2 f2 d2 f2 bb1 f2] [c2 g2 c3 g2 e2 g2 c2 g2]
        ...
        .hpf(200).lpf(saw.range(3500, 5000).slow(64)).lpe(1.2).lpq(2.5).adsr("0.005:0.35:0.5:0.12")
        .orbit(5),
    ...
    chord("<Am Dm <Bb [Bb|F]> C Gm [F|F|Dm] Am Dm>").voicing(rank = sine.range(0, 1.8).fast(7).add(perlin.range(0, 0.3)))
        ...
        .gain(0.13).orbit(6).room(0.4).rsize(6),
    ...
    note("<a5 d6 bb5 c6 g5 f5 a5 d6>")
        ...
        .gain(saw.range(0.0, 0.6).slow(64)).body("glass")
