...
export bassShape = (p) => p
    ...
    .orbit(1).gain(0.18).adsr("0.007:0.5:0.7:0.10").lpadsr("0.007:2.0:0.0:0.10").pan(0.2)
    .superimpose(
      x => x.pan(0.8),
      x => x.transpose("<0 12 0 -12>/8").pan(0.5).superimpose(pan(0.8))
    ).phaser(1/13).phaserdepth(0.15).phasercenter(2000).phasersweep(1200)
    .spread(sine.range(0.1, 0.4).early(1.5).slow(24)).hpf(260).lpf(3200).lpe(1.2)
    .lpq(berlin.range(1.5, 2.2).seg(32).slow(32))

// Sub voice: tremoloed triangle, soft distortion, pedal engine.
export subShape = (p) => p
    .slow(2).orbit(2).clip(0.5).sound("tri").gain(0.7).distort("0.3:tube:2").postgain(0.50).analog(2.5)
