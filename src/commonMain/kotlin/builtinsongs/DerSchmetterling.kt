...
stack(                                                                                                         //////////////          //////////////
  ...
  n(`<[-7 0 2 4] [-7 0 4 [2 6]|[4 2]|2|2|2] [-5 -1 2 4] [-6 -1 [4 3]|5|3|3|3 [1 -1]|1|1|1|1]>*2`)                  //          DISCO!          //
    ...
    .gain(1.00).distort("0.350:tube:4").postgain("<0.700!48 0.330!16>") // . solo()                                     //       //      //
    .adsr("0.01:3.0:0.0:0.01").adsrCurves("exp:exp:exp").clip(0.8)  // . mute()                                        //     //    //    //
    .release("<0.20!16 0.35!16>").vibrato(6).vibmod(0.05)                                                             //   //          //  //
  ...
  n(`<[7 4 2 <-1 4 1 3> [0 -1 -3 -1] [0 -3] -2 <[-1 5@3] [5 6@3] [4 9@3] [4 3@3]>]!4
    ...
    .sound("supersaw").unison(9).detune(0.08)
    .distort("1.0:tube:4").distort(1.0).vibrato(4).vibmod(0.02)
    .adsr("0.005:1.5:0.1:0.005").adsrCurves("exp:exp:exp").lpadsr("0.005:1.0:0.2:0.005")
    .clip("<0.94!31 0.9 0.94!31 0.825>".fast(2))
    .gain(0.6).postgain(0.35).hpf("<280!48 400!16 280!48 400!16>").lpf(saw.range(1,0).pow(2.0).mul(500).add(1000).slow(4)).lpe(3.0).lpq(1.25)
    // .coarse(2).coarseos(4)  
  ...
  n("<0 0 2 4 0 0 -2 -1>") //   . solo()
    ...
    .analog(feel).sound("supersaw").unison(5).detune(0.07)
    .hpf(170).hpq(1.0).lpf(1000).lpe(3.0).lpq(1.25)
    .adsr("0.007:1.5:0.0:0.005").adsrCurves("exp:exp:exp").lpadsr("0.007:1.0:0.3:0.005").velocity("1.00 0.9!3 0.95 0.90!3".fast(4))
    .clip("<0.93!31 0.9 0.93!31 0.825>".fast(2)).gain(0.8).distort("1:tube:2").distort(1.0)
    // .coarse(2).coarseos(4)  
    .pan(0.35).postgain(0.28).superimpose(
      x => x.pan(0.65),
      x => x.orbit(3).postgain(0.25).hpf(170).lpf(1000) // .lpe(1.0)
  ...
  n("<0 0 2 4 0 0 -2 -1>").struct("<[x!1]!32 [x@3 x]!32 [x!4]!64>").fast(2).velocity("1.00 0.9!3 0.95 0.90!3".fast(4)) // . mute()
    .scale("<e2:minor!88 e2:minor!8>").sound("saw").gain(0.5).distort("0.2:tube:1").coarse(2).postgain(0.55).clip(0.9)
    .adsr("0.010:0.8:0.1:0.03").adsrCurves("exp:exp:exp").lpadsr("0.010:0.5:0.0:0.15").hpf(60).hpq(1.25).lpf(100).lpe(1.5).hpq(1.25) //  .solo()
  ...
  // Master
).room("0.2:1").roomlp(500).roomdim(500)
