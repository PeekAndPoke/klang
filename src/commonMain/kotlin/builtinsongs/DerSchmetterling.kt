...
export guitar1_shape = x => x.gain(0.5).velocity(guitarDyna.fast(2)).sound(guitar).adsrOff().unison(voices = 15, spread = 0.05) // . solo()
  ...
  .oscp("low", 1.0).oscp("lowHz", "1500").oscp("lowQ", 0.6)
...
export guitar1_arrange = x => x.orbit(1) //  . solo()
  .scale("<e3:minor!48 e4:minor!16 e3:minor!48 e4:minor!16>").postgain(0.220)  // .mute()
...
export guitar2_shape = x => x.gain(0.5).velocity(guitarDyna.fast(2)).sound(guitar).adsrOff().unison(voices = 13, spread = 0.05)
  ...
  .oscp("low", 1.5).oscp("lowHz", 1000).oscp("lowQ", 0.6)
  .oscp("mid", 3.5).oscp("midHz", 1750).oscp("midQ", 0.5)
  .oscp("high", 1.75).oscp("highHz", 3000).oscp("highQ", 0.7)
  .hpf(200)
  .clip(guitarClip.fast(2)).pan(0.3).body(material = "oak", wet = 0.3)

export guitar2_arrange = x => x.orbit(2)  // . solo()
  .scale("<e2:minor>").postgain(0.180).mute("<0!128 1!16 0!16>") // .mute()
...
export guitar3_shape = x => x.gain(0.5).velocity(guitarDyna.fast(2)).sound(guitar).adsrOff().unison(voices = 11, spread = 0.05)
  ...
  .oscp("mid", 3.5).oscp("midHz", 1550).oscp("midQ", 0.5)
  .oscp("high", 1.75).oscp("highHz", 3000).oscp("highQ", 0.7)
  .hpf(200)
  .clip(guitarClip.fast(2)).pan(0.7).body(material = "rosewood", wet = 0.3)
...
export hats_shape = x => x.gain(0.42).pan(0.40)
  .hpf(5000).lpf(freq = "14000".add(perlin.mul(250).fast(4)), q = 0.7).adsr(0.003, 0.05, 0.80, 1.0)
...
export clap_shape = x => x.gain(0.090).pan(0.2).superimpose(pan(0.8))
