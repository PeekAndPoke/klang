...
stack(                                                                                                                       //////////////          //////////////.
  ...
  n(`<[-7 0 2 4] [-7 0 4 [2 6]|[4 2]|2|2|2] [-5 -1 2 4] [-6 -1 [4 3]|5|3|3|3 [1 -1]|1|1|1|1]>*2`)                                //          DISCO!          //.
    ...
    .hpf(1450).lpf(1550).lpe(berlin.range(2, 2.10).fast(4)).lpq(3.0).lpadsr("0.007:1.5:0.3:0.01")                                    //                  //.
    .gain(0.50).distort("0.520:tube:4").postgain("<0.200!48 0.100!16 0.200!48 0.300!16>") // . solo()                                 //       //      //.
  ...
  n(`<[7 [4@4 2 -1] 2 1 [0 -1 -3 -1] [0 -3] -2 <[-1 5@3] [5 6@3] [[4 5] 8@3] [[3 4] 3@3]>]!4
    ...
    .sound("supersaw").unison(9).spread(0.07).gain(0.75).postgain(0.115).distort("1:tube:4").distort(0.80)    
    .clip("<0.86!31 0.77 0.86!31 0.85 0.86!30 0.81 0.72>".fast(2)).adsr("0.005:2.5:0.0:0.031").lpadsr("0.005:0.8:0.0:0.011")    
    .hpf("<550!16 450!16 550!16 700!16>").lpf("3000".add(saw.range(1, 0).pow(1.8).mul(1000)).slow(8)).lpe(1.0).lpq(1.5)
  ...
  n("<0 0 2 4 0 0 -2 -1>")  //  . solo()
    ...
    .sound("supersaw").unison(7).spread(0.10).gain(0.75).postgain(0.115).distort("1:tube:4").distort(0.85)
    .clip("<0.86!31 0.77 0.86!31 0.85 0.86!30 0.81 0.72>".fast(2)).adsr("0.005:2.5:0.0:0.032").lpadsr("0.005:0.7:0.0:0.01")    
    .hpf(110).lpf(2700).lpe(1.0).lpq(1.4)
    .coarse(2).coarseos(4).pan(0.3).superimpose(
      x => x.pan(0.7),
      x => x.postgain(0.10).hpf(300).lpf(2800).scaleTranspose("<4!7 [2 [3 4@3]]!1 4!7 [-7 -3] 4!7 [2 [3 4@3]]!1 4!7 [-3 [2 4@3]]>")
           .pan(0.2).superimpose(pan(0.8))
    ).superimpose(hpf(3000).lpf(5400).lpq(0.7).postgain(0.05)).mute("<0!128 1!16 0!16>").pipeline("pedal").body("maple").bodyMix(0.2)
  , // Bass
  n("<0 0 2 4 0 0 -2 -1>").struct("<[x!1]!16 [x@3 x]!48 [x!4]!80>").fast(2).velocity("0.98 0.94 0.96 0.94".fast(2))  // . mute()
    .orbit(3).scale("e1:minor").sound("saw").gain(0.5).distort("0.05:soft:2").postgain(0.24).clip(0.65)
    .adsr("0.007:4.3:0.1:0.015").lpadsr("0.007:0.075:0.0:0.01").hpf(65).hpq(1.0).lpf(200).lpe(25).lpq(1.0)  //  .solo()
  ...
  sound("<[~!2]!2  [~!4]!2  [~!8]!2  [~!16]  [~!24]  [~  sd  ~ sd]!32 [~ sd ~ sd]!32>").mute("<0!128 1!32>")  // . solo()
    .pan(0.485).late(0.002).orbit(5).gain(0.31).hpf(350).lpf(11500).adsr("0.002:0.20:0.2:0.2")
  ...
  sound("<[hh hh hh hh]!16 [hh hh oh hh]!24 [cr hh cr hh]!24 [~ rd ~ rd]!32>").fast(2).mute("<0!128 1!32>") // . solo()
    .pan(0.515).late(0.0005).orbit(5).gain(0.31).hpf(800).lpf("9000".add(perlin.mul(300))).adsr("0.005:0.15:0.8:0.2"), // . mute()
  sound("brown!8").orbit(6).coarse(16).gain(0.4).hpf(2000).pan(sine.range(0.4, 0.6).slow(11)).adsr("0.005:0.2:0.4:0.02") //  .solo(),
  // Master
).analog(feel).room("0.10:8:0.12").seed(timeOfDay.mul(60*60*24))
