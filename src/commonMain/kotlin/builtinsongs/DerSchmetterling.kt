...
stack(                                                                                                         //////////////          //////////////
  ...
  n(`<[-7 0 2 4] [-7 0 4 [2 6]|[4 2]|2|2|2] [-5 -1 2 4] [-6 -1 [4 3]|5|3|3|3 [1 -1]|1|1|1|1]>*2`)                  //          DISCO!          //
    ...
    .hpf(1250).lpf(1600).lpe(berlin.range(1.8, 1.8)).lpq(1.8).lpadsr("0.001:2.0:0.2:0.01")                             //                  //
    .gain(1.00).distort("0.320:tube:4").postgain("<0.700!48 0.330!16>") // . solo()                                     //       //      //
    .adsr("0.01:3.0:0.0:0.01").adsrCurves("exp:exp:exp").clip(0.8)  // . mute()                                        //     //    //    //
    .release("<0.20!16 0.35!16>").vibrato(5).vibmod(0.03)                                                              //   //          //  //
    .phaser(1/8).phaserdepth(0.05).phasersweep(500).phasercenter(1800)                                               // //               // //
  ...
  n(`<[7 4 2 <-1 4 1 3> [0 -1 -3 -1] [0 -3] -2 <[-1 5@3] [5 6@3] [4 9@3] [4 3@3]>]!4
    ...
    .distort("1.0:tube:4").distort(1.1).vibrato(4).vibmod(0.03)
    .adsr("0.005:1.5:0.1:0.005").adsrCurves("exp:exp:exp").lpadsr("0.005:1.0:0.4:0.005")
    .clip("<0.95!31 0.9 0.95!31 0.825>".fast(2))
    .gain(0.6).postgain(0.35).hpf("<320!48 700!16 320!32 700!32>").lpf(saw.range(1,0).pow(1.8).mul(800).add(1200).slow(4)).lpe(1.5).lpq(1.25)
    .pan(0.2).superimpose(pan(0.8))
    .engine("pedal").body("wood").bodyMix(0.5)
  ,
  // Guitar 2
  n("<0 0 2 4 0 0 -2 -1>")  //  . solo()
    .orbit(2).scale("<e2:minor>").struct("<[x!8]!14 [x!12]!2 [x!8]!32>").fast(2)
    .analog(feel).sound("supersaw").unison(13).detune(0.07)
    .hpf(180).lpf(1500).lpe(1.5).lpq(1.15).vibrato(3).vibmod(0.03)
    .adsr("0.008:2.0:0.0:0.005").adsrCurves("exp:exp:exp").lpadsr("0.008:1.0:0.0:0.005").velocity("1.00 0.9!3 0.95 0.90!3".fast(4))
    .clip("<0.95!31 0.9 0.95!31 0.825>".fast(2)).gain(0.8).distort("1:tube:4").distort(1.1)
    .coarse(2).coarseos(4)  
    .pan(0.35).postgain(0.26).superimpose(
      x => x.pan(0.65),
      x => x.orbit(2).postgain(0.26).hpf(180).lpf(1500) // .lpe(1.0)
            .scaleTranspose("<4!7 [2 [3 4@3]]!1 4!7 [-3 [-4 -3@3]]>").pan(0.3).superimpose(pan(0.7))
    ).mute("<0!128 1!16 0!16>").engine("pedal").body("wood").bodyMix(0.5)
  , // Bass
  n("<0 0 2 4 0 0 -2 -1>").struct("<[x!1]!32 [x@3 x]!32 [x!4]!64>").fast(2).velocity("1.00 0.9!3 0.95 0.90!3".fast(4)) // . mute()
    .scale("<e2:minor!88 e2:minor!8>").sound("saw").gain(0.5).distort("0.2:tube:1").coarse(2).postgain(0.475).clip(1.0)
    .adsr("0.015:1.0:0.0:0.03").adsrCurves("exp:exp:exp").hpf(100).lpf(120).lpe(1).lpadsr("0.007:1.0:0.0:0.15") //  .solo()
    .mute("<0!128 1!32>") // .engine("pedal")
  , // Drums
  sound("<[bd!2]!2 [bd!4]!2 [bd!8]!2 [bd!16] [bd!24] [bd ~  bd ~ ]!24 [bd bd bd bd]!24>").mute("<0!128 1!32>")  // . solo()
    .early(0.001).orbit(5).gain(0.85).hpf(80).lpf(6000).adsr("0.005:0.15:0.1:0.1"),
  sound("<[~!2]!2  [~!4]!2  [~!8]!2  [~!16]  [~!24]  [~  sd ~  sd]!24 [~  sd ~  sd]!24>").mute("<0!128 1!32>") // . solo()
    .early(0.001).orbit(5).gain(0.90).hpf(180).lpf(6000).adsr("0.006:0.25:0.1:0.1").superimpose(bandf(200).bandq(4).gain(0.4)),
  sound("<[hh hh oh hh]!48 [cr hh cr hh]!16 [0 hh 0 hh]!16>").fast(2).mute("<0!128 1!32>")  //  . solo()
    .late(0.003).orbit(5).gain(0.80).hpf(4000).lpf(6500).adsr("0.01:0.2:0.5:0.2") // . mute()
  // Master
).room("0.1:1").roomlp(500).roomdim(500)
