...
stack(                                                                                                         //////////////          //////////////
  ...
  n(`<[-7 0 2 4] [-7 0 4 [2 6]|[4 2]|2|2|2|2] [-5 -1 2 4] [-6 -1 [4 3]|5|3|3|3 [1 -1]|1|1|1|1]>*2`)                //          DISCO!          //
    ...
    .hpf(1350).lpf(1600).lpe(berlin.range(2.0, 2.0)).lpq(1.2).lpadsr("0.005:3.0:0.5:0.10")                             //                  //
    .gain(1.00).distort("0.300:tube:4").postgain("<0.850!48 0.350!16>") // . solo()                                     //       //      //
    .adsr("0.01:3.0:0.0:0.10").adsrCurves("exp:exp:exp").clip(0.8)  // . mute()                                        //     //    //    //
    .release("<0.15!16 0.25!16>")                                                                                     //   //          //  //
    .phaser(1/8).phaserdepth(0.1).phasersweep(1000).phasercenter(1800)                                               // //               // //
  ...
  n(`<[7 4 2 <-1 4 1 3> [0 -1 -3 -1] [0 -3] -2 <[-1 4@3] [5 6@3] [4 7@3] [4 6@3]>]!4
    ...
    .adsr("0.005:3.0:0.0:0.005").adsrCurves("exp:exp:exp").lpadsr("0.007:1.0:0.0:0.005")
    .clip("<0.96!31 0.9 0.96!31 0.825>".fast(2))
    .gain(0.6).postgain(0.3).hpf("<360!48 600!16 360!32 600!32>").lpf(saw.range(1,0).pow(1.5).mul(1000).add(1400).slow(4)).lpe(1.0).lpq(1.50)
  ...
  n("<0 0 2 4 0 0 -2 -1>")  //  . solo()
    ...
    .analog(feel).sound("supersaw").unison(15).detune(0.05)
    .hpf(180).lpf(1200).lpe(1.5).lpq(1.25)
    .adsr("0.009:3.0:0.0:0.005").adsrCurves("exp:exp:exp").lpadsr("0.007:1.0:0.0:0.005").velocity("1.00 0.95!3 0.98 0.95!3".fast(2))
    .clip("<0.96!31 0.9 0.96!31 0.825>".fast(2)).gain(0.8).distort("1:tube:4").distort(1.0)
    .coarse(2).coarseos(4)  
    .pan(0.4).postgain(0.285).superimpose(
      x => x.pan(0.6),
      x => x.postgain(0.25).hpf(200).lpf(1200) // .lpe(1.0)
  ...
  n("<0 0 2 4 0 0 -2 -1>").struct("<[x!1]!32 [x@3 x]!32 [x!4]!64>").fast(2).velocity("1.00 0.95!3 0.98 0.95!3".fast(2))
    ...
    .adsr("0.007:0.3:0.0:0.15").adsrCurves("exp:exp:exp").hpf(80).hpq(1).lpf(100).lpe(1).lpadsr("0.007:0.2:0.0:0.15") //  .solo()
    .mute("<0!128 1!32>") // .engine("pedal")
  , // Drums
  sound("<[bd!2]!2 [bd!4]!2 [bd!8]!2 [bd!16] [bd!24] [bd ~  bd ~ ]!24 [bd bd bd bd]!24>").mute("<0!128 1!32>") // . solo()
    .early(0.002).orbit(5).gain(0.80).hpf(80).lpf(5000).adsr("0.005:0.15:0.1:0.1"),
