...
stack(                                                                                                         //////////////          //////////////
  ...
  n(`<[-7 0 2 4] [-7 0 4 [2 6]|[4 2]|2|2|2] [-5 -1 2 4] [-6 -1 [4 3]|5|3|3|3 [1 -1]|1|1|1|1]>*2`)                 //           DISCO!         //
    ...
    .hpf(1250).lpf(1520).lpe(berlin.range(1.8, 1.8)).lpq(1.8).lpadsr("0.005:3.0:0.5:0.01")                            //                  //
  ...
  n(`<[7 4 2 <-1 4 1 3> [0 -1 -3 -1] [0 -3] -2 <[-1 4@3] [5 6@3] [4 7@3] [4 6@3]>]!4
    ...
    .adsr("0.004:1.5:0.1:0.005").adsrCurves("exp:exp:exp").lpadsr("0.004:1.0:0.0:0.005")
    .clip("<0.96!31 0.9 0.96!31 0.825>".fast(2))
    .gain(0.6).postgain(0.33).hpf("<300!48 600!16 300!32 600!32>").lpf(saw.range(1,0).pow(1.8).mul(1000).add(1400).slow(4)).lpe(1.0).lpq(1.25)
  ...
  n("<0 0 2 4 0 0 -2 -1>")  //  . solo()
    ...
    .adsr("0.007:1.5:0.0:0.005").adsrCurves("exp:exp:exp").lpadsr("0.007:1.0:0.0:0.005").velocity("1.00 0.95!3 0.98 0.95!3".fast(2))
    .clip("<0.97!31 0.9 0.965!31 0.825>".fast(2)).gain(0.8).distort("1:tube:4").distort(1.0)
    .coarse(2).coarseos(4)  
    .pan(0.35).postgain(0.27).superimpose(
      x => x.pan(0.65),
      x => x.late(0.002).postgain(0.24).hpf(180).lpf(1400) // .lpe(1.0)
  ...
  n("<0 0 2 4 0 0 -2 -1>").struct("<[x!1]!32 [x@3 x]!32 [x!4]!64>").fast(2).velocity("1.00 0.95!3 0.98 0.95!3".fast(2))
    ...
    .adsr("0.005:1.0:0.0:0.15").adsrCurves("exp:exp:exp").hpf(80).hpq(1).lpf(100).lpe(1).lpadsr("0.007:0.2:0.0:0.15") //  .solo()
  ...
  sound("<[hh hh oh hh]!48 [cr hh cr hh]!16 [0 hh 0 hh]!16>").fast(2).mute("<0!128 1!32>")  // . solo()
    .late(0.004).orbit(5).gain(1.00).hpf(3000).lpf(6200).adsr("0.01:0.2:0.5:0.2")
  // Master
