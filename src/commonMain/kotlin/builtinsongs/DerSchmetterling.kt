...
let guitar = (() => {
  ...
  let pStrQ       = Osc.param("strQ",          2.000, "String brightness")
  ...
  let signal = Osc.supersaw(freq = Osc.freq(), voices = 35, spread = 0.10)
    ...
    .phasePool(on = 1, kMin = 0.30, kMax = 0.70, warmup = 0, selection = "normal:0.5:0.1")
    // character knobs — plain scalars, SuperSaw-typed, must precede the filter
    .analog(pAnalog).spreadPower(4.0).sideAtten(0.5).gainJitter(0.20).centerJitter(0.66)
    ...
    .lowpass(cutoffHz = Osc.freq().mul(pStrFloor.plus(pStrSweep.adsr(pAttack, pStrDecay, 0.0, pRelease))), q = pStrQ)
      .analog(pAnalog)    
    // noise burst
    .plus(Osc.whitenoise().highpass(1000).lowpass(10000).adsr(0.0, 0.05, 0.0, 0.005).mul(0.2))
...
export lead_shape = x => x.gain(0.40).velocity(guitarDyna.fast(2))
  ...
  .oscp("hptrack", Math.pow(2, 3/12)).oscp("hpQ", 0.707)
  .oscp("mids", 3.0).oscp("midsQ", 1.9).oscp("midsHz", leadHz).oscp("presence", 0.0)
...
export guitar1_shape = x => x.gain(0.8).velocity(guitarDyna.fast(2)).sound(guitar).unison(15).spread(0.05) // . solo()
  .oscp("hptrack", Math.pow(2, 9/12)).oscp("hpq", 0.85)
  .oscp("mids", 1.5).oscp("midsHz", 1360).oscp("midsQ", 0.7)
  .oscp("presence", 1.0).oscp("presenceHz", 3000).oscp("presenceQ", 1.5)
...
export guitar2_shape = x => x.gain(0.8).velocity(guitarDyna.fast(2)).sound(guitar).unison(13).spread(0.05)
  ...
  .oscp("mids", 5.0).oscp("midsHz", 1050).oscp("midsQ", 0.7)
  .oscp("presence", 1.5).oscp("presenceHz", 2600).oscp("presenceQ", 1.3)
...
export guitar3_shape = x => x.gain(0.8).velocity(guitarDyna.fast(2)).sound(guitar).unison(11).spread(0.05)
  ...
  .oscp("mids", 2.0).oscp("midsHz", 800).oscp("midsQ", 0.7)
  .oscp("presence", 1.5).oscp("presenceHz", 2300).oscp("presenceQ", 4.0)
  .clip("<0.97!31 0.93 0.97!31 0.92 0.96!30 0.88 0.92>".fast(2))
...
export bass_shape = x => x.gain(1.0).velocity("0.98 0.96 0.97 0.96".fast(2)).sound(bass).postgain(0.09) //. mute()
    .oscp("drive", 0.30).oscp("grindlo", 120).oscp("grindhi", 500).oscp("grind", 0.75).oscp("sub", 0.90).hpf(25) // . solo()
