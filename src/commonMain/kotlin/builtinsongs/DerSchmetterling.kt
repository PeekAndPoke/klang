...
stack(                                                                                                         //////////////          //////////////
  ...
  n(`<[-7 0 2 4] [-7 0 4 [2 6]|[4 2]|2|2|2|2] [-5 -1 2 4] [-6 -1 [4 3]|5|3|3|3 [1 -1]|1|1|1|1]>*2`)                //          DISCO!          //
    ...
    .hpf(1200).lpf(2400).lpe(berlin.range(1.1, 1.2)).lpq(1.2).lpadsr("0.01:2.0:0.0:0.05")                              //                  //
    .gain(0.50).distort("1.0:tube:4").postgain(0.800) // . solo()                                                       //       //      //
  ...
  // Guitar 1
  n(`<[7 4 2 <4 -1 4 3> [0 -1 -3 -1] [0 -3] -2 <[3 4@3] [5 6@3] [6 5@3] [4 6@3]>]!4
      [[4 2] [-1 -3] 0 [2 [2 6@3]]]!2 [[0 -3] [-1 -3] 0 <[4 6] [2 3]>] [<7 [7 4 3 1 -1 -3  -4 -7]> [-5 -6] -7 [-2 <3 -1>]]>/4`) // . solo()
    .scale("<e2:minor!48 e3:minor!16>").struct("<[x!16]!7 [x!24]!1 [x!16]!16>") // .mute()
    .velocity("1.00 0.95!3 0.98 0.95!3".fast(2)).analog(feel)
    .sound("supersaw").unison(15).detune(0.10)
    .distort("1.0:tube:4").distort(1.0)
    .adsr("0.005:5.0:0.0:0.005").adsrCurves("square:exp:cube").lpadsr("0.007:3.0:0.0:0.005")
    .clip("<0.98!31 0.92>".fast(2))
    .gain(0.6).postgain(0.35).hpf(400).lpf(saw.range(3000, 1500).slow(4)).lpe(0.5).lpq(2.5).scaleTranspose(7).pan(0.3).superimpose(pan(0.7))
    .orbit(1).engine("pedal")
  ,
  // Guitar 2
  n("<0 0 2 4 0 0 -2 -1>") // . solo()
    .scale("<e2:minor>").struct("<[x!8]!14 [x!12]!2 [x!8]!32>").fast(2)
    .analog(feel).sound("supersaw").unison(11).detune(0.07)
    .hpf(140).hpq(1.0).lpf(2000).lpe(1.0).lpq(2.0)
    .adsr("0.005:5.0:0.0:0.005").adsrCurves("square:exp:cube").lpadsr("0.007:3.0:0.0:0.005").velocity("1.00 0.95!3 0.98 0.95!3".fast(2))
    .clip("<0.98!31 0.92>".fast(2)).gain(0.8).distort("1:tube:4").distort(1.0)
    .coarse(2).coarseos(2)  
    .pan(0.35).postgain(0.3).superimpose(
      x => x.pan(0.65),
      x => x.unison(4).postgain(0.3).hpf(180).lpf(2000) // .lpe(1.0)
  ...
  sound("<[~!2]!2  [~!4]!2  [~!8]!2  [~!16]  [~!24]  [~  sd ~  sd]!24 [~  sd ~  sd]!24>").mute("<0!128 1!32>")// . solo()
    .early(0.002).orbit(5).gain(0.95).hpf(180).lpf(7500).adsr("0.006:0.25:0.1:0.1").superimpose(bandf(500).gain(0.8)),
