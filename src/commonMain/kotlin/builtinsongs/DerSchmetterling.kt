...
stack(                                                                                                         //////////////          //////////////
  ...
  n("<[-7 0 2 4] [-7 0 4 2] [-5 -1 2 4] [-6 -1 3 1]>*2")                                                           //          DISCO!          //
    ...
    .hpf(800).lpf(2200).lpe(2.0).lpadsr("0.02:0.25:0.40:0.11").warmth(0.1)                                             //                  //
    ...
    .superimpose(                                                                                                    // //               // //
      x => x.shuffle("<1!64 0!16 1!1 4/8!14 1!33>").seed(sinOfNight.add(1).mul(24 * 60 * 15))                       //                       //
        .superimpose(transpose(12).detune(0.10).velocity("<0!32 0.20!32>").pan(0.2).early(0.001))
        .superimpose(transpose(12).detune(0.15).velocity("<0!32 0.20!32>").pan(0.8).late(0.001))
  ...
  n(`<[7 4 2 <4 -1 4 3> [0 -1 -3 -1] [0 -3] -2 <[-1 4@3] [5 6@3] [1 2@3] [2 5@3]>]!4 
    ...
    .lpf(300).lpe(3.0).hpf("<240!48 360!16>")
    .distort("1.2:tube:4").distort(sine.range(0.4, 0.7).slow(10*32))
    .adsr("0.01:0.15:0.66:0.105").lpadsr("0.025:0.25:0.66:0.15").clip(0.50).crush(saw.range(8, 4).slow(32)) // . solo()
    .gain(0.3).postgain(0.9).orbit(1).pan(0.4).late(0.001)
    .superimpose(
      x => x.lpadsr("0.023:0.3:0.5:0.15").hpf(500).lpf(2800).scaleTranspose(7).postgain(0.9).pan(0.2).lpe(0.5).superimpose(pan(0.8).early(0.001))
    )
  ,
  // Bass
  n("<0 0 2 4 0 0 -2 -1>").struct("<[x!8]!14 [x!12]!2 [x!8]!32>").fast(2).velocity("1.02 0.95!3 0.98 0.95!3".fast(2))
    ...
    .lpf(700).lpe(3.0).hpf(80).distort("1.2:tube:4")
    .adsr("0.01:0.15:0.66:0.10").lpadsr("0.01:0.03:0.66:0.15").clip(0.50)  // . solo()
    .gain(0.3).postgain(0.75).orbit(2).pan(0.5).mute("<0!128 1!16 0!16>")
    .superimpose(
      x => x.postgain(0.55).pan(0.33).hpf(350).lpf(2800).lpe(0.5).adsr("0.025:0.15:0.66:0.13").lpadsr("0.021:0.02:0.8:0.15")
            .scaleTranspose("<4!15 [2 4]>").superimpose(pan(0.66).late(0.002))
    )
  ,
  // Drums
  sound("<[bd!2]!2 [bd!4]!2 [bd!8]!2 [bd!16] [bd!24] [bd ~  bd ~ ]!24 [bd bd bd bd]!24>").mute("<0!128 1!32>")// . solo()
    .early(0.002).orbit(3).gain(0.95).hpf(100).lpf(5000).adsr("0.009:0.15:0.5:0.1"),
  sound("<[~!2]!2  [~!4]!2  [~!8]!2  [~!16]  [~!24]  [~  sd ~  sd]!24 [~  sd ~  sd]!24>").mute("<0!128 1!32>")// . solo()
    .early(0.002).orbit(3).gain(0.7).hpf(180).lpf(7000).adsr("0.009:0.15:0.5:0.1").superimpose(bandf(200).gain(0.35)),
  sound("<[hh hh oh hh]!48 [cr hh cr hh]!16 [0 hh 0 hh]!16>").fast(2).mute("<0!128 1!32>")  // . solo()
    .late(0.004).orbit(4).gain(0.90).hpf(800).lpf(7000).adsr("0.01:0.2:0.5:0.2")
  // Master
).compressor("-6:3:10:0.02:0.25").engine("pedal").room("0.02:5")
