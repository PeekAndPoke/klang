...
let supersawHp = (() => {
  ...
  let signal = Osc.supersaw(freq = Osc.freq(), voices = pVoices, spread = pSpread)
    ...
    .analog(pAnalog).spreadPower(2.0).sideAtten(0.6).gainJitter(0.25).centerJitter(0.05)
    // Add overtones 1 octave up
    .add(Osc.supersaw(freq = Osc.freq().mul(2), voices = pVoices.div(2), spread = pSpread).centerJitter(0.05).mul(1/8))  
    // Add overtones 2 octaves up
    .add(Osc.supersaw(freq = Osc.freq().mul(4), voices = pVoices.div(3), spread = pSpread).centerJitter(0.05).mul(1/16))  
    // Follow the frequency to avoid low mud
    .highpass(Osc.freq().mul(pHpTrack), pHpQ)
   
  return signal
    .add(signal.bandpass(700, 0.500).mul(2.5))
    .add(signal.bandpass(1500, 0.500).mul(2.0))
    .lowpass(5200)
...
stack(                                                                                                                  
  ...
  n(`<[-7 0 2 4] [-7 0 4 [2 6]|[4 2]|2|2|2] [-5 -1 2 4] [-6 -1 [4 3]|[5 3]|3|3|3 [1 -1]|1|1|1|1]>*2`)                  
    ...
    .hpf(800).lpf(1800).lpe(perlin.range(1.9, 2.1).fast(2)).lpq(1.5).lpadsr("0.010:0.5:0.3:0.03")                      
  ...
  n(`<[0 [0@4 [4 7] -3] -1 -3 [0 2 4 3] 0 2 <[-1 1 3@2] [[3 4] 6@2 7] [[1 3] 4 3 2] [[6 12 7 5]]>]!4
    ...
    .sound(supersawHp).unison(7).spread(0.07).gain(0.5).postgain(0.14).distort("1:tube:4").distort(0.80)    
    .clip("<0.93!31 0.85 0.93!31 0.83 0.93!30 0.85 0.80>".fast(2)).adsr("0.004:3.0:0.0:0.040").lpadsr("0.004:1.0:0.0:0.025")    
    .hpf("<600>").lpf("3500".add(saw.range(1, 0).pow(4.0).mul(200)).slow(8)).lpe(1.2).lpq(0.8)
  ...
  n("<0 0 2 4 0 0 -2 -1>") //   . solo()
    ...
    .clip("<0.93!31 0.85 0.93!31 0.83 0.93!30 0.85 0.80>".fast(2)).adsr("0.009:3.0:0.0:0.040").lpadsr("0.009:1.0:0.0:0.025")    
    .hpf(90).lpf(3200).lpe(1.0).lpq(0.8)
    .coarse(2).coarseos(2).pan(0.65).superimpose(
      x => x.pan(0.35),
      x => x.postgain(0.13).hpf(200).lpf(3200).transpose(12).scaleTranspose("<4!7 [2 [3 4@3]]!1 4!7 [0 -3] -3!7 [-5 [-4 -3@3]]!1 -3!7 [-3 [2 4@3]]>")
  ...
  n("<0 0 2 4 0 0 -2 -1>").struct("<[x!1]!16 [x@3 x]!48 [x!4]!80>").fast(2).velocity("0.98 0.94 0.96 0.94".fast(2))  // . mute()
    .orbit(3).scale("e1:minor").sound("sine").gain(1.0).distort("0.08:soft:4").postgain(0.075).clip(0.66)
    .adsr("0.010:4.0:0.0:0.010").lpadsr("0.005:0.06:0.0:0.01").hpf(50).lpf(300).lpe(20).lpq(0.7)  //  .solo()
    .pan(0.35).superimpose(pan(0.65)).superimpose(transpose(12).postgain(0.055)).mute("<0!128 1!32>") // .pipeline("pedal")
  , // Drums
  ...
  sound("<[~!2]!2  [~!4]!2  [~!8]!2  [~!16]  [~!24]  [~  sd  ~ sd]!32 [~ sd ~ sd]!32>").mute("<0!128 1!32>") // . solo()
    .pan(0.5).late(0.0015).orbit(5).pan(0.475).gain(0.20).hpf(180).lpf(13500).adsr("0.002:0.05:0.8:0.2")
  ...
  sound("pink!8").orbit(7).gain(0.05).hpf(8000).lpf(perlin.range(14000, 15000)).lpq(0.5)
    .pan(sine.range(0.4, 0.6).slow(11)).adsr("0.005:0.15:0.0:0.05")  // .solo()
  // Master
  ,master(Master.of(MasterFx.reverb().wet(0.04).damp(0.7).roomSize(7), MasterFx.gain(1.50)))
).analog(feel).seed(timeOfDay.mul(10*60*60*24)).shuffle("<1!80 2!48 1!128 2!64>").swingBy(0.01, 4)
