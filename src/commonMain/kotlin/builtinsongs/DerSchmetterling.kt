...
import * from "sprudel"                                                                                                         ////
                                                                                                                               //  //
let feel = 5.0    // 0.0 .. cold | 10.0 .. warm                                                                               //    //
                                                                                                                             //      //
                                                                                                                            //        //
stack(                                                                                                         //////////////          //////////////
  // Lead                                                                                                        //                              //
  n(`<[-7 0 2 4] [-7 0 4 [2 6]|[4 2]|2|2|2] [-5 -1 2 4] [-6 -1 [4 3]|5|3|3|3 [1 -1]|1|1|1|1]>*2`)                  //          DISCO!          //
    ...
    .hpf(1600).lpf(1750).lpe(berlin.range(1.6, 1.7).fast(4)).lpq(2.2).lpadsr("0.010:1.5:0.4:0.01")                     //                  //
    .gain(0.50).distort("0.550:tube:4").postgain("<0.600!48 0.290!16>") // . solo()                                     //       //      //
  ...
  n(`<[7 4 2 <-1 3 1 3> [0 -1 -3 -1] [0 -3] -2 <[-1 5@3] [5 6@3] [[4 5] 8@3] [[3 4] 3@3]>]!4
    ...
    .clip("<0.88!31 0.80 0.88!31 0.91 0.88!30 0.80 0.73>".fast(2)).adsr("0.003:4.0:0.0:0.02").adsrCurves("exp:exp:exp").lpadsr("0.003:2.0:0.0:0.02")    
    .hpf("<450!48 750!16>").hpq(1.2).lpf(1200).lpe(2).lpq(1.2).warmth(0.1)
  ...
  n("<0 0 2 4 0 0 -2 -1>")  //  . solo()
    ...
    .sound("supersaw").unison(11).detune(0.10).gain(0.8).postgain(0.24).distort("1:diode:4").distort(0.8)
    .clip("<0.88!31 0.80 0.88!31 0.91 0.89!30 0.80 0.73>".fast(2)).adsr("0.003:4.0:0.0:0.012").adsrCurves("exp:exp:exp").lpadsr("0.003:2.0:0.0:0.02")    
    .hpf(120).hpq(1.2).lpf(1100).lpe(2).lpq(1.2).warmth(0.1)
    .coarse(2).pan(0.35).superimpose(
      x => x.pan(0.65),
      x => x.orbit(3).postgain(0.23).scaleTranspose("<4!7 [2 [3 4@3]]!1 4!7 [-3 [-4 -3@3]]>").pan(0.3).superimpose(pan(0.7))
  ...
  n("<0 0 2 4 0 0 -2 -1>").struct("<[x!1]!16 [x@3 x]!48 [x!4]!48>").fast(2).velocity("1.00 0.93!3 0.95 0.93!3".fast(2)) // . mute()
    .orbit(4).coarse(2).scale("<e2:minor!88 e2:minor!8>").sound("saw").gain(0.5).distort("0.4:tube:1").postgain(0.60).clip(0.7)
    .adsr("0.003:2.0:0.0:0.015").adsrCurves("exp:exp:exp").lpadsr("0.003:0.3:0.0:0.01").hpf(70).hpq(1).lpf(180).lpe(1.0) // .solo()
    .mute("<0!128 1!32>") // .engine("pedal")
  , // Drums
  sound("<[bd!2]!2 [bd!4]!2 [bd!8]!2 [bd!16] [bd!24] [bd  ~ bd  ~]!24 [bd bd bd bd]!24>").mute("<0!128 1!32>")  // . solo()
    .orbit(5).gain(0.90).hpf(80).lpf(5000).adsr("0.003:0.2:0.5:0.5"),
  ...
  sound("<[hh hh hh hh]!16 [hh hh oh hh]!16 [cr hh cr hh]!24 [~ hh ~ hh]!24>").fast(2).mute("<0!128 1!32>")  //  . solo()
    .late(0.001).orbit(7).gain(0.70).hpf(3000).lpf(5700).adsr("0.005:0.3:0.7:0.5") // . mute()
  // Master
).room("0.1:5:0.1")
 .compressor("-6:2.5:7:0.01:0.15").seed(timeOfDay.mul(60*60*24))
