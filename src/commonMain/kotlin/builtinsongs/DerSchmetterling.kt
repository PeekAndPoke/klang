...
stack(                                                                                                         //////////////          //////////////
  ...
  n("<[-7 0 2 4] [-7 0 4 2] [-5 -1 2 4] [-6 -1 3 1]>*2")                                                           //          DISCO!          //
    ...
    .hpf(800).lpf(1500).lpe(3.5).lpadsr("0.02:0.2:0.65:0.1").warmth(0.3)                                              //                  //
    .gain(0.70).distort("0.1:gentle:2").postgain(0.55) // . solo()                                                      //       //      //
    ...
    .superimpose(                                                                                                    // //               // //
      x => x.shuffle("<1!64 0!16 1!1 4/8!14 1!33>").seed(sinOfNight.add(1).mul(24 * 60 * 15))                       //                       //
        ...
        .superimpose(transpose(12).detune(0.20).velocity("<0!96 0.10!32>").lpf(2100).pan(0.7))
    ).phaser(1/8).phaserdepth("<0.05!64 0.4!16 0.1!48>").phasersweep(1000).phasercenter(1500).mute("<0!192 1!32>"),
  // Pad
  n("<[0 0 2 <4 -1 4 3> 0 0 -2 <-1 3 6 -2>]!4 [0 [2 4] 0 [2 [2 -1@3]]]!2 [0 [6 4] 0 <[5 6] [-2 0]>] [<0 4> [2 1] 0 [-2 <-1 -4>]]>/4")
    ...
    .lpf("<1400!48 1900!16>").lpq(0.8).lpe(2.0).hpf("<220!48 300!16>")
    .distort("1.5:tube:4").distort(sine.range(0.3, 0.6).slow(10*32))
    .adsr("0.015:0.15:0.66:0.15").lpadsr("0.015:0.175:0.66:0.15").clip(0.55).crush(saw.range(8, 4).slow(32)) // . solo()
    .gain(0.3).postgain(1.5).orbit(1).pan(0.4).late(0.001)
    .superimpose(
      x => x.lpadsr("0.013:0.3:0.5:0.15").scaleTranspose(7).postgain(1.1).pan(0.2).hpf(400).lpe(1.1).superimpose(pan(0.8))
    )
  ,
  // Bass
  n("<0 0 2 4 0 0 -2 -1>").struct("<[x!8]!14 [x!12]!2 [x!8]!32>").fast(2).velocity("1.02 0.95!3 0.98 0.95!3".fast(2))
    ...
    .lpf(saw.range(800, 1100).slow(4)).lpe(2.0).hpf(90).distort("1.5:tube:4")
    .adsr("0.012:0.15:0.66:0.15").lpadsr("0.012:0.15:0.5:0.15").clip(0.55) //  . solo()
    .gain(0.3).postgain(1.3).orbit(2).pan(0.5).mute("<0!128 1!16 0!16>")
    .superimpose(
      x => x.lpf(saw.range(1000, 1300).slow(4)).lpe(1.0).lpadsr("0.015:0.15:0.5:0.15")
            .scaleTranspose("<4!8  4!2 [4 4 4 7] 4  4!2 [4 4 4 0] [2 4]>").postgain(0.7).pan(0.33).hpf(200).superimpose(pan(0.66))
    )
  ,
  // Drums
  sound("<[bd!2]!2 [bd!4]!2 [bd!8]!2 [bd!16] [bd!24] [bd sd bd sd]!24 [bd [bd,sd] bd [bd,sd]]!24>").mute("<0!128 1!32>")// . solo()
    .early(0.002).orbit(3).gain(0.95).crush(10).crushos(2).hpf(80).lpf("2000:1:1.5").lpe("<2!128 2.3!128 2.7!128>").adsr("0.007:0.2:0.5:0.1"),
  sound("<[hh hh oh hh]!48 [cr hh cr hh]!16 [0 hh 0 hh]!16>").fast(2).mute("<0!128 1!32>") // . solo()
    .late(0.004).orbit(4).gain(0.90).crush(10).crushos(2).hpf(800).lpf("3200:1:1.25").adsr("0.01:0.2:0.5:0.2").lpadsr("0.01:0.2:0.5:0.2")
  // Master
