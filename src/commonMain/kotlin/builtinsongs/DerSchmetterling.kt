...
export guitarClip = "<0.93!31 0.85 0.93!31 0.83 0.93!30 0.75 0.80>".sub(perlin.range(0.0, 0.02).seg(8)).mul("<0.985!32 [1.1 0.99!7]!32 0.975!32 [1.1 0.99!7]!32>")
...
export lead_shape = x => x.gain(0.5).sound("supersaw").unison(voices = 7, spread = 0.1)
  ...
  .clip(1.8).velocity(guitarDyna).body("violin")
  .hpf(1600).lpf(freq = 1600, env = 10.0, attack = 0.02, decay = 1.0, sustain = 0.5, release = 0.100, q = 5.0)
  .pan(sine.range(0.1, 0.9).fast(29))
...
export bass = n(bass_pat).struct("<[x!2]!16 [x!2 ~ x]!16 [x x@2 x]!28 [x!8]!2 [[x x] x!3]!2>").fast(2).apply(bass_shape).tag("bass")
...
export hats_shape = x => x.gain(0.27).pan(0.40)
  .hpf(5000).lpf(freq = "14500".add(perlin.mul(250).fast(4)), q = 0.7).adsr(0.003, 0.05, 0.80, 1.0)
...
export shaker_shape = x => x.gain(0.18).velocity("<1.0 0.90 0.95 0.90>*16") // . mute()
  .hpf(freq = 7000, q = 0.5).lpf(freq = 15000, q = 0.7) //  . solo()
  .pan(sine.range(0.3, 0.8).slow(8)).adsr(0.015, 0.15, 0.0, 0.01)
