...
let supersawHp = (() => {
  ...
  let signal = Osc.supersaw(freq = Osc.freq(), voices = pVoices, spread = pSpread)
    ...
    .analog(pAnalog).spreadPower(2.0).sideAtten(0.5).gainJitter(0.25).centerJitter(0.05)
    // Add overtones 1 octave up
    .add(Osc.supersaw(freq = Osc.freq().mul(2), voices = pVoices.div(2), spread = pSpread).centerJitter(0.05).mul(0.25))  
    // Add overtones 2 octaves up
    .add(Osc.supersaw(freq = Osc.freq().mul(4), voices = pVoices.div(3), spread = pSpread).centerJitter(0.05).mul(0.125))  
    .highpass(Osc.freq().mul(pHpTrack), pHpQ, pAnalog)
   
  return signal
    .add(signal.bandpass(800, 0.500).mul(1.0))
...
stack(                                                                                                                  
  ...
  n(`<[-7 0 2 4] [-7 0 4 [2 6]|[4 2]|2|2|2] [-5 -1 2 4] [-6 -1 [4 3]|[5 3]|3|3|3 [1 -1]|1|1|1|1]>*2`)                  
    ...
    .gain(0.50).distort("0.470:tube:4").postgain("<0.200!48 0.110!16 0.200!48 0.280!16>")  // . solo()                  
  ...
  n(`<[0 [0@4 [4 7] -3] -1 -3 [0 2 4 3] 0 2 <[-1 1 3@2] [[3 4] 6@2 7] [[1 3] 4 3 2] [[6 12 7 5]]>]!4
    ...
    .clip("<0.93!31 0.85 0.93!31 0.83 0.93!30 0.85 0.80>".fast(2)).adsr("0.004:3.0:0.0:0.045").lpadsr("0.004:1.0:0.0:0.025")    
    .hpf("<600>").lpf("3500".add(saw.range(1, 0).pow(4.0).mul(200)).slow(8)).lpe(1.2).lpq(0.6)
    .pan(0.3).superimpose(pan(0.7)).body("spruce").bodyMix(0.4)
  , // Guitar 2
  n("<0 0 2 4 0 0 -2 -1>") //   . solo()
    ...
    .sound(supersawHp).unison(9).spread(0.08).gain(0.5).postgain(0.15).distort("1:tube:4").distort(0.80)
    .clip("<0.93!31 0.85 0.93!31 0.83 0.93!30 0.85 0.80>".fast(2)).adsr("0.009:3.0:0.0:0.045").lpadsr("0.009:1.0:0.0:0.025")    
    .hpf(80).lpf(3200).lpe(1.0).lpq(0.6)
    .coarse(2).coarseos(2).pan(0.65).superimpose(
      x => x.pan(0.35),
      x => x.postgain(0.14).hpf(160).lpf(3200).transpose(12).scaleTranspose("<4!7 [2 [3 4@3]]!1 4!7 [0 -3] -3!7 [-5 [-4 -3@3]]!1 -3!7 [-3 [2 4@3]]>")
  ...
  n("<0 0 2 4 0 0 -2 -1>").struct("<[x!1]!16 [x@3 x]!48 [x!4]!80>").fast(2).velocity("0.98 0.94 0.96 0.94".fast(2))  // . mute()
    .orbit(3).scale("e1:minor").sound("sine").gain(1.0).distort("0.08:soft:4").postgain(0.080).clip(0.66)
    .adsr("0.010:4.0:0.0:0.010").lpadsr("0.005:0.06:0.0:0.01").hpf(50).lpf(500).lpe(10).lpq(0.7)  //  .solo()
    .pan(0.35).superimpose(pan(0.65)).superimpose(transpose(12).postgain(0.05)).mute("<0!128 1!32>") // .pipeline("pedal")
  , // Drums
