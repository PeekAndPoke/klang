...
let guitar = (() => {
  ...
  let pRelease    = Osc.param("release",       0.027, "Release")

  // String params — both dimensionless harmonic counts, no absolute Hz anywhere
  let pStrFloor   = Osc.param("strFloor",     12.000, "String lowpass floor, in harmonics of the note")
  let pStrSweep   = Osc.param("strSweep",     24.000, "Extra harmonics alive at the pluck")
  let pStrDecay   = Osc.param("strDecay",      0.100, "How fast the string darkens")
  let pStrQ       = Osc.param("strQ",          2.707, "String brightness")
  ...
  let amped = signal   
    ...
    .eq()
      ...
      .highpass(50)                                               // filter the low end
...
export lead_arrange = x => x.orbit(0)
  .scale("<e4:minor!48 e5:minor!16 e4:minor!48 e3:minor!16>").postgain("<0.220!48 0.100!16 0.220!48 0.245!16>")  
...
// Guitar 1  --------------------------------------------------------------------------------------------------------------------------------------------------
export guitar1_pat =
  `<[[0 0] [[2 3] [4 2] 0] 2 <4 3 1> [0 2 4 3] 0 2 <[-1 1 3@2] [[3 4] 6@2 7] [[1 3] 4 3 2] [[6 10 7 5]]>]!4
...
export guitar1_shape = x => x.gain(0.8).velocity(guitarDyna.fast(2)).sound(guitar).adsrOff().unison(15).spread(0.05) // . solo()
  ...
  .oscp("mids", 20.0).oscp("midsHz", 1400).oscp("midsQ", 2.0)
  .oscp("presence", 3.5).oscp("presenceHz", "3500".sub(saw.pow(0.5).mul(1500).slow(8))).oscp("presenceQ", 1.0)
...
export guitar2_shape = x => x.gain(0.775).velocity(guitarDyna.fast(2)).sound(guitar).adsrOff().unison(13).spread(0.05)
  .oscp("hptrack", Math.pow(2, 0/12)).oscp("hpq", 1.5)
  .oscp("mids", 25.0).oscp("midsHz", 1000).oscp("midsQ", 1.75)
  .oscp("presence", 3.5).oscp("presenceHz", 3000).oscp("presenceQ", 1.0)
...
export guitar3_shape = x => x.gain(0.75).velocity(guitarDyna.fast(2)).sound(guitar).adsrOff().unison(11).spread(0.05)
  .oscp("hptrack", 1.0).oscp("hpq", 1.5)
  .oscp("mids", 25.0).oscp("midsHz", 750).oscp("midsQ", 1.5)
  .oscp("presence", 3.0).oscp("presenceHz", 2400).oscp("presenceQ", 1.0)
...
export bass_shape = x => x.gain(1.0).velocity("0.98 0.96 0.97 0.96".fast(2)).sound(bass).postgain(0.16) //. mute()
    .oscp("drive", 0.80).oscp("grindlo", 100).oscp("grindhi", 1000).oscp("grind", 0.80).oscp("sub", 0.80).hpf(25) // . solo()
