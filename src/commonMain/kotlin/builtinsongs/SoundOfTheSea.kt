...
internal val soundOfTheSeaSong = Song(
    ...
    code = """
        ...
        .hpf(300).bandf(perlin.range(110, 110 * 15).slow(64)).bandq(perlin.range(0.5, 5.0).slow(21))

        , // Water ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
       sound("pink pink pink pink").legato(2).degrade(0.5).adsr("1.5:3.0:0.5:5.0") //  . solo()
       .gain(water).hpf(120).lpf(4000).lpe(2).bandf(300).bandq(1.0).early(2)

        , // Waves ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        note("~ c ~ c ~ c ~").legato(4).degrade(96/100).sound("pink").adsr("1.0:5.0:0.6:20.0") // . solo()
        .gain(waves).hpf(120).lpf(5000).lpadsr("1.0:3.0:0.4:20.0").lpenv(5)
          ...
          .superimpose(x => x.sound("pink").adsr("0.3:0.8:0.2:1.5").velocity(0.2).hpf(2500).lpf(8000))
             ...
             , // Windspiel ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
               n(randrun(16)).orbit(2).fast(4).degradeBy(0.995).sound("glockenspiel").scale("f2:pentatonic").pan(0.3).body("glass")
                 .gain(0.25).distort(0.05).postgain(windSpiel).adsr("0.05:0.2:0.5:5.0").hpf(400).lpf("3000:2:3.0")
                     .orbit(1).delay(0.25).delaytime(pure(1/4).div(cps)).delayfeedback(0.85) // . solo()
                           ).room(0.25).rsize(10.0).rlp(12500).seed(sinOfDay.add(1).mul(24 * 60 * 60 * 100)).analog(2)
