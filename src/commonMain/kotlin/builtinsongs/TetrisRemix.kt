...
export bass = note(bassPattern).slow(2).struct("[x!16]")
    .sound("supersaw").unison(32).spread(0.15).onepole(23846).clip(0.95)
    .gain(0.4).adsr(0.01, 0.2, 0.3, 0.1).pan(0.5).superimpose(pan(0.1).transpose(12), pan(0.9).transpose(12))
    .hpf(300).lpf(1150).lpe(19).lpq(1.3).distort(0.8, "soft", 2).postgain(0.115).pipeline("pedal")
    .orbit(2)  // .solo()
...
export skank = chord("<Am Em F Am <Em Em Dm G#> Em Am <G E F G>>").voicing()
    ...
    .hpf(1000).lpf(8000).lpadsr(0.02, 0.1, 0.0, 0.1).lpe(21.7).onepole(22309)
...
export sub = tetrisSub.struct("x!1 [~!1 x!1?] x!5 ~!1").orbit(7).clip(0.8).distort(0.3)
  .lpadsr(0.005, 3.0, 0.0, 0.2).hpf(60).lpf(300).lpadsr(0.005, 1.0, 0.0, 0.2).lpe(48).postgain(0.34) // .solo()
