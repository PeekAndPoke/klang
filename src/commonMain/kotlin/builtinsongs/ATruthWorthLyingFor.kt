...
let guitar = Osc.register("guitar", (() => {
  ...
  let signal = Osc.supersaw(freq = Osc.freq(), voices = 3, freqSpread = pSpread, analog = pAnalog).mul(0.3)
    .plus(Osc.zawtooth(freq = Osc.freq()).mul(0.10))          // Zawtooth for raw grit
    .plus(Osc.zawtooth(freq = Osc.freq().mul(2)).mul(0.10))   // Zawtooth overtones for more grit
    .plus(Osc.square(freq = Osc.freq().mul(3.001)).mul(0.03)) // Square for aggressive mid bite

  return signal
    .lowpass(Osc.sine(0.5).plus(1).times(500).plus(pBrightness), 1.25)              // Pre-distortion: sweeping lowpass adds dynamic character
    .plus(signal.mul(0.20).highpass(110).lowpass(300))                              // Re-add low end after bandpass
...
stack( // Gitarre! ----------------------------------------------------------------------------
  ...
  n(`<   [0 0 0 7] [0 5 0 2] [0 3 0 5] [0 3 0 0]  [ 0 0 0 7] [0  5 0 8] [0 7 0 5] [ 0 7 0 0]
    ...
    .orbit(1).fast(4).scale("C3:chromatic").pan(0.3).notchf("1200").lpf("4500").clip(0.975)  // .solo()
    .s(guitar).oscp("drive", drive * 0.9).oscp("brightnes", 5000).oscp("spread", 0.03).hpf(250).postgain(0.23)
    .transpose(tp).filterWhen(t => t % stay > 16)
  , // Melody 2 --------------------------------------------------------------------------------------------------
  n(`<   [0 0 0 7] [0 5 0 2] [0 3 0 5] [0 3 0 0]  [ 0 0 0 7] [0  5 0 8] [0 7 0 5] [ 0 7 0 0]
         [0 0 0 7] [0 5 0 2] [0 3 0 5] [0 3 0 0]  [12 0 0 0] [0 10 0 7] [0 8 7 8] [10 8 7@2]>`)
    .orbit(2).fast(4).scale("C4:chromatic").pan(0.7).notchf("1300").lpf("5050").clip(0.975)  // . solo()
    .s(guitar).oscp("drive", drive * 0.9).oscp("brightnes", 5000).oscp("spread", 0.03).hpf(400).postgain(0.20)
  ...
  ).orbit(3).fast(1).scale("C2:chromatic").gain(1.0).clip(1.00).release(0.15).hpf(160).postgain(0.20)
    .s(guitar).oscparam("drive", drive).oscp("brightness", 4500).oscp("spread", 0.12)
  ...
  ).orbit(4).scale("C2:chromatic").clip(0.8).sound("tri").gain(1.5).warmth(0.2).distort("0.5:soft:2").postgain(0.215)
    .adsr("0.005:0.2:0.5:0.075").tremolo("0.1:8::0:0").hpf(80).lpf(1000).velocity("<[1.0 0.9 0.9 0.9]>")
  ...
  ).orbit(5).early(0.002).adsr("0.0075:0.3:0.2:1.0 0.01:0.2:0.2:1.0".slow(16)).n(0).gain(1.0).hpf(100).filterWhen(t => t % stay >= 3.95)  // .solo()
  , // Drums 1 ------------------------------------------------------------------------------------------------
  s("<[cr hh!7]!7 [cr hh!3 [hh hh] [hh hh] [cr hh] [oh hh]]>")
    .orbit(6).late(0.002).adsr("0.01:0.2:0.8:0.3").gain(0.95).hpf(800).velocity("<[1.0 0.95 0.95 0.95]>") // .solo()
