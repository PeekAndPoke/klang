...
stack( // Gitarre! ----------------------------------------------------------------------------
  morse("Gitarre!").n("-5").scale("c4:chromatic").sound("tri").clip(0.5).orbit(7).fast(2).transpose(tp).analog(2)
    .gain(0.8).distort(1).warmth(0.3).postgain("0.25 0.10 0.175 0.35".slow(stay)).hpf(1800).lpf(2450).lpe(2).pan(0.5) // .solo()
  ,// Melody 1 ---------------------------------------------------------------------------------
  n(`<   [0 0 0 7] [0 5 0 2] [0 3 0 5] [0 3 0 0]  [ 0 0 0 7] [0  5 0 8] [0 7 0 5] [ 0 7 0 0]
    ...
    .orbit(1).fast(4).scale("C3:chromatic").hpf(500).lpf(3400).lpe(1.5).lpq(1.50).clip(0.96) // .solo()
    .s(guitar).oscp("drive", drive * 0.9).oscp("brightness", 6200).oscp("spread", 0.07).postgain(0.08).body("cedar").bodyMix(0.5)
    .transpose(tp).pan(0.25).superimpose(pan(0.75)).velocity("<[1.0 0.95 0.975 0.95]>").filterWhen(t => t % stay > 16)
  , // Melody 2 --------------------------------------------------------------------------------------------------
  n(`<   [0 0 0 7] [0 5 0 2] [0 3 0 5] [0 3 0 0]  [ 0 0 0 7] [0  5 0  8] [0 7 0 5] [ 0 7 0 0]
         [0 0 0 7] [0 5 0 2] [0 3 0 5] [0 3 0 0]  [12 0 0 0] [0 10 0 7] [0 8 7 8] [10 8 7@2]>`)
    .orbit(2).fast(4).scale("C4:chromatic").hpf(800).lpf(3400).lpe(1.5).lpq(1.75).clip(0.96).late(0.001)  // . solo()
    .s(guitar).oscp("drive", drive * 0.9).oscp("brightness", 6200).oscp("spread", 0.06).postgain(0.07).body("cedar").bodyMix(0.5)
  ...
  ).orbit(3).fast(1).scale("C2:chromatic").clip(0.9925).hpf(120).lpf(2800).lpe(1.25).lpq(1.75).postgain(0.11)
    .s(guitar).oscparam("drive", drive).oscp("brightness", 6000).oscp("spread", 0.10).body("wood").bodyMix(0.5) //  . mute()
  ...
  ).orbit(4).scale("C2:chromatic").clip(0.95).sound("saw").gain(1.5).distort("0.8:tube:2").analog(1).postgain(0.13)
    .adsr("0.005:0.5:0.2:0.025").lpadsr("0.005:0.1:0.0:0.075").hpf(80).lpf(200).lpe(20.0).lpq(1).velocity("<[1.0 0.95 0.975 0.95]>")
  ...
  ).orbit(6).early(0.002).adsr("0.005:0.2:0.2:0.5").gain(0.6).hpf(90).lpf(11500)
    .superimpose(bandf(195).bandq(1.1).gain(0.2)).filterWhen(t => t % stay >= 3.95)  // .solo()
  , // Drums 1 ------------------------------------------------------------------------------------------------
  s("<[cr hh!7]!7 [cr hh!3 [hh hh] [hh hh] [oh hh] [oh hh]]>")
    .orbit(7).late(-0.003).adsr("0.007:0.2:0.9:0.7").gain(0.75).hpf(800).lpf(13500).velocity("<[1.0 0.95 0.975 0.95]>") // .solo()
