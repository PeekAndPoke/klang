...
stack(
  // Claps --------------------------------------------------------------------------------------------------------------------
  sound("cp ~ cp ~ ~ cp cp ~  cp ~ ~ ~ cp cp ~ ~").slow(4).orbit(0).gain(0.25).legato(2.0)
  ...
  n("0").morse("Schön ist es auf der Welt zu sein!").orbit(0)
    .scale("C5:major").scaleTranspose("0 -2 2 2".slow(32)).bandf(2000).bandq(7.0).hpf(1000).analog(5)
  ...
  n("<[0 2 4 6 7 6 4 2]!14 [0 -1 0 4 6 9 7 6] [-2 -1 0 2 7 4 -1 -3]>") // .solo()
    ...
    .orbit(1).s("supersaw").unison(5).detune(saw.range(0.0, 0.35).slow(16)).spread(1.0 )
    .gain(0.5).distort(0.5).warmth(0.1).postgain(0.25).adsr("0.01:0.1:0.5:0.1").clip(1.1)
  ...
  , // Bass -----------------------------------------------------------------------------------------------------------------------------
  note("<a1 [f1 c2 e1 [f1 c2]] [a1 [c2 f1] a1 [f1@3 e1]] [a1@2 c2@3 d2 [c2,c3] [d1,d1,d2]]>/4").clip(0.7).struct("x!4").slow(16)
    .orbit(2).s("supersaw").unison(9).detune(saw.range(0.05, 0.45).slow(64)).warmth(0.01) // . mute()
    .gain(1.0).adsr("0.01:0.6:0.8:2.90").postgain(0.45).coarse(2).coarseos(2) // solo()
    .superimpose(
      x => x.orbit(3).scaleTranspose("<[12 12 7 12 12 [12 12] 0 -12] [12 12 0 12 12 [0 12] 0 -12]>/32")
        .pan(sine.range(0.15, 0.8).slow(32)).clip(0.8)
