...
stack(
  ...
  n("0").morse("Schön ist es auf der Welt zu sein!").orbit(0).analog(5)
    ...
    .sound("pulse").warmth(0.75).crush(5).gain(0.03).clip(0.35).pan(berlin.slow(2)).adsr("0.03:0.08:0.2:0.1") // .solo()
    .filterWhen(x => x >= wait * 12 && x < (wait * 6 + keep))
  , // Melody -----------------------------------------------------------------------------------------------------------------
  n("<[0 2 4 6 7 6 4 2]!14 [-3 -1 0 4 6 4 6 7] [-2 -1 0 7 4 2 0 -1]>")
    ...
    .gain(0.6).distort(0.25).warmth(0.02).postgain(0.3).adsr("0.01:0.6:0.7:0.155")
    .pan(sine.range(0.3, 0.7).slow(16)) // . solo()
    .superimpose(bandf(notch).bandq(1.0).gain(0.125).transpose(12)).body("wood")
    .hpf(500).lpf(950).lpenv(perlin.range(3.0, 4.5).lpq(3.5).slow(8)).analog(2)
    .filterWhen(x => x >= wait * 4 && x < (wait * 4 + keep)) // . solo()
  , // Bass -----------------------------------------------------------------------------------------------------------------------------
  note("<a1 [f1 c2 e1 [d2 c2]] [a1 [c2 f1] a1 [f1@3 e1]] [a1@2 c2@3 d2 [[c2,g2] [c2,c3]] [d1,d1,a2,d2]]>/4").clip(0.8).struct("x!4").slow(16)
    ...
    .superimpose(
      x => x.gain(saw.range(0.1, 1.0).slow(64).pow(1.5).mul(1.8)).vibrato("12.5".add(berlin2)).vibmod(0.15)
        .crush("2.0".add(berlin2.mul(0.75).slow(2))).crushos(4).hpf(300).lpf(6.0 * 440).lpq(3.0).lpe(0.1).postgain(0.6)
