...
stack(
  ...
  note("<a1 [f1 c2 e1 [f2 c2]] [a1 [c2 f1] a1 [f1@3 e1]] [a1@2 c2@3 d2 [c2,c3] [d1,d1,d2]]>/4").clip(0.8).struct("x!4").slow(16)
    ...
    ).lpf(4 * 440).lpq(2.5).hpf(80).notchf(notch).notchq(0.75).analog(2).body("glass")
    .superimpose(
      x => x.gain(saw.range(0.2, 1.0).slow(64).pow(1.25).mul(2.5)).vibrato("0.5".add(perlin.div(20))).vibmod(0.1)
    ...
    .analog(7).filterWhen(x => x < (wait * 4 + keep))
  , // Perc 2 ------------------------------------------------------------------------------------------------------------------
