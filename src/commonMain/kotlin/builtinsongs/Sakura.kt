...
let pad = Osc.supertri(freq = Osc.freq(), voices = 5).analog(0.3)
      .lowpass(cutoffHz = Osc.sine(0.3).plus(2).times(200).plus(Osc.freq()), q = 2, analog = Osc.slot.analog)
...
stack(
  ...
  `).sound(koto).legato(0.8).slow(14).gain(0.7)
    .superimpose(fast(2).gain(0.095).pan(0.3))
  ...
  `).sound(shaku).slow(14).gain(0.25).pan(perlin.range(0.3, 0.7).slow(24))
  ...
  ,stack(
    // Root
    note("a2  d2  a2  f2  c2  e2  a2").sound(pad).slow(14).legato(1.02).gain(0.20).pan(0.4)
    // Third (minor/major character)
    ,note("c3  f2  c3  a2  e2  gs2 c3").sound(pad).slow(14).legato(1.02).gain(0.20).pan(0.7)
    // Fifth
    ,note("e3  a2  e3  c3  g2  b2  e3").sound(pad).slow(14).legato(1.02).gain(0.20).pan(0.2)
    // Octave
    ,note("a3  d3  a3  f3  c3  e3  a3").sound(pad).slow(14).legato(1.05).gain(0.15).pan(0.8)
    // High third
    ,note("c4  f3  c4  a3  e3  gs3 c4").sound(pad).slow(14).legato(1.05).gain(0.15).pan(0.3)
    // High fifth
    ,note("e4  a3  e4  c4  g3  b3  e4").sound(pad).slow(14).legato(1.05).gain(0.15).pan(0.6)
