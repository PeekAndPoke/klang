...
stack(                                                                                                    //////////////          //////////////
  ...
  n("<[-7 0 2 4] [-7 0 4 2] [-5 -1 2 4] [-6 -1 3 1]>*2")                                                      //          DISCO!          //
    ...
    .apply(                                                                                                     // //               // //
      ...
      x => x.shuffle("<1!64 0!16 1!1 4/8!14 1!33>").seed(sinOfNight.add(1).mul(24 * 60 * 15))            
        .superimpose(transpose(12).detune(0.07).velocity("<0!32 0.25!32>").lpf(4200).pan(0.8))          
        .superimpose(transpose(24).detune(0.10).velocity("<0!96 0.095!32>").lpf(5000).pan(0.2))  
  ...
  n("<[0 0 2 4 0 0 -2 -1]!4 [0 [2 4] 0 [2 [2 -1@3]]]!2 [0 [6 4] 0 <[2 3] [-2 -0]>] [4 [2 1] 0 [-2 <-1 -4>]]>/4")
    ...
    .notchf(snd).notchq(0.5).lpf("<1900!48 2400!16>").hpf(200).warmth(saw.range(0.3, 0.1).slow(5*32))
    .distort("0.3:gentle:4").distort(sine.range(0.25, 0.35).slow(256))
    .adsr("0.01:0.25:0.5:0.045").clip(0.75).crush(saw.range(8, 4).slow(32)) //   . solo()
    .gain(0.37).orbit(1).pan(0.3),
  // Bass
  n("<0 0 2 4 0 0 -2 -1>").struct("<[x!8]!14 [x!12]!2 [x!8]!32>").fast(2).velocity("1.02 0.95!3 0.98 0.95!3".fast(2))
    .scale("d#2:minor").sound("saw").warmth(saw.range(0.5, 0.1).slow(5*32))
    .notchf(snd).notchq(0.5).lpf("1300").hpf(140).distort("0.3:soft:4")
    .adsr("0.01:0.2:0.5:0.05").clip(0.75)  // . solo()
    .gain(0.4).orbit(2).pan(0.7),
  // Drums
  sound("<[bd!2]!2 [bd!4]!2 [bd!8]!2 [bd!16] [bd!24] [bd sd bd sd]!24 [bd [bd,sd] bd [bd,sd]]!8>")
    .orbit(3).gain(0.8).crush(6).crushos(2).hpf(100).lpf("2400:1").lpe("<2!128 2.3!128 2.7!128>").adsr("0.01:0:1.0:0.1"),
  sound("<[hh hh oh hh]!24 [cr hh cr hh]!16>") //. solo()
    .orbit(4).gain(0.375).fast(2).crush(7).crushos(2).hpf(500).lpf("3500:1:2").adsr("0.01:0:1.0:0.1")
  // Master
