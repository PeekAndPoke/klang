    ...
    sections = listOf(
        ...
        TutorialSection(
            ...
            blocks = listOf(
                Block.Markdown(
                    markdown = """
                    `transpose()` shifts every note by **semitones**: the smallest move there is — from any note to the very next one, cracks included — and twelve of them make one octave. (A semitone is smaller than the walking step of ${Tut.firstNotes}: a step to the next letter crosses one semitone or two, depending on the letters.) So `transpose(12)` lifts the whole line one floor up, and `transpose(-12)` drops it one floor down.
