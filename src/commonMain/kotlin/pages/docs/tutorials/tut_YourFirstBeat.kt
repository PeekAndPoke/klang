        ...
        TutorialSection(
            ...
            blocks = listOf(
                Block.Markdown(
                    ...
                    **Try it:** swap the `sd` for a `cp` and press **Update** — same beat, different flavour.
                    """.trimIndent(),
                ),
                Block.Code(
                    code = """
        ...
        TutorialSection(
            ...
            blocks = listOf(
                Block.Markdown(
                    ...
                    **Try it:** this is your first `//` swap. Put `//` in front of the first line, remove it from the second, and press **Update**. Keep exactly one line live — if both are, only the lower one plays.

                    **Listen for:** the kick comes back around at exactly the same moment in both versions — three hits spread out to fill the time four filled. (The counter confirms it: same pace either way.) The cycle is a fixed container — you never changed it, only how many pieces it is cut into.
                    """.trimIndent(),
                ),
                Block.Code(
                    code = """
