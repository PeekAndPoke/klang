    ...
    sections = listOf(
        TutorialSection(
            ...
            text = "note() works exactly like sound() — names in quotes, spaces splitting the cycle into " +
                ...
                "generates the tone itself, at whatever pitch note() asks for. Sine is the plainest " +
                "voice Klang has, a pure tone with nothing added; its siblings — saw, square, triangle — " +
            ...
            code = """
                note("c3 e3 g3 c4")  // four pitches, four steps — the rule you already know
                  ...
                  .gain(0.5)         // pure tones carry a lot of energy — keep them well below full
            """.trimIndent(),
        ),
        TutorialSection(
            ...
            text = "Notes are named with seven letters — c d e f g a b — and then the ladder starts " +
                ...
                "repeat.",
            code = """
                note("c3 d3 e3 f3 g3 a3 b3 c4")  // up the whole ladder, back to c
                  .sound("sine").gain(0.5)
            """.trimIndent(),
        ),
        TutorialSection(
            ...
            text = "The number after the letter picks the floor: c3 is low, c5 is high. Same letter, " +
                "same note — different height. Swap the // between the two lines and press Update.\n\n" +
                "Listen for: the first line is one note changing size; the second is four different " +
                "notes. That is the octave from the last section, now side by side with ordinary " +
                "movement.\n\nOctaves 1 and 2 are bass territory; 3 and 4 are where melodies usually " +
                "live; 5 and up cuts through everything. Try it: drop the first c3 to c2 and press " +
            ...
            code = """
                ...
                // note("c3 d3 e3 d3").sound("sine").gain(0.5)  // four neighbouring notes on one floor — swap to compare
            """.trimIndent(),
        ),
        TutorialSection(
            heading = "The notes between",
            text = "Between most letters sits one more note — the black keys on a piano. You write " +
                "them by adding s for sharp (a half step up) or b for flat (a half step down): cs3 " +
                "sits between c3 and d3, eb3 between d3 and e3.\n\nThis is the ladder from before " +
                "with one change: the e3 dropped to eb3. Listen for: the third step lands lower — " +
                "into the crack between d3 and e3 — and the whole ladder suddenly sounds darker. " +
                "Which of these in-between notes belong together with which letters is what scales " +
                "are about; that is a later lesson.",
            code = """
                note("c3 d3 eb3 f3 g3 a3 b3 c4")  // the ladder again — e3 lowered to eb3
                  .sound("sine").gain(0.5)
            """.trimIndent(),
        ),
        TutorialSection(
            ...
            text = "Melodies move in two ways: to a neighbouring letter, or further. Musicians call " +
                ...
                "Swap the // between the two lines and press Update.\n\nListen for: steps walk — " +
            ...
            code = """
                ...
                // note("c4 g4 e4 c5").sound("sine").gain(0.5)  // leaps: the same voice, suddenly singing
            """.trimIndent(),
        ),
        TutorialSection(
            heading = "A first melody",
            text = "Steps, one leap, and two rests — every idea from this lesson and the last two in " +
                "one line. Let it loop; it is written to circle back on itself. The walking part in " +
            ...
            code = """
                note("a3 c4 d4 ~ e4 d4 c4 ~")  // leap up, walk to the top, walk back down
                  ...
                  .gain(0.5)                   // pure tones read loud — 0.5 sits level with the drums' 0.8
            """.trimIndent(),
        ),
    ),
)
